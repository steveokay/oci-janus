...
> **Last updated:** 2026-07-16 — Threat-modelled + security-reviewed `feat/signing-coverage-rollup` (`git diff 5613e6d..8936dd1`, main..HEAD) — the new read-only `GET /api/v1/signing/coverage` BFF endpoint in `services/management` + the Security → Signing coverage FE tab. **1 new finding logged: SEC-090 (INFO), verdict PASS — SHIP.** Pure gRPC orchestration (no proto change, no migration, no new secret, no mutation). All six focus areas clear: reader-allowed access bar is precedented (matches FE-API-020 `handleSecurityOverview` + reader-allowed trusted-keys/recent-signers; discloses nothing beyond the already-reader-visible `/repositories` inventory); tenant_id is server-derived from the validated JWT/API-key and threaded into all four downstream calls (no header/body tenant source, no cross-tenant path — cache key `(tenant_id,window)` cannot be poisoned); `?window=` is the only input (Atoi, reject <1, cap 200), fan-out bounded (8 workers / 1000-repo cap / 3s per-call / 60s cache); Warn logs carry only non-secret digests + public key_ids + repo labels; fail-open is correct for a visibility endpoint that gates no admission decision. 0 CRITICAL / 0 HIGH / 0 MEDIUM / 0 LOW; 2 INFO fast-follows (SEC-090-A expired-cache-entries never reaped — bounded to ~200 entries single-tenant; SEC-090-B keep the rollup's visibility model in lockstep if per-repo read-scoping is ever added). No merge blocker.
>
> **Last updated:** 2026-07-13 — Tracker truth-up (no new review). **SEC-087 (LOW) flipped OPEN → RESOLVED** — the fix (`AND tenant_id = $1` on the `ListRepositories` artifact-types secondary query) already shipped inline in PR #321 and is verified in-code at `services/metadata/internal/repository/repository.go:289-291`; the entry was stale while the status.md #321 row already recorded it. No open MEDIUM/HIGH remain; open findings are unchanged from the 2026-07-04 count minus SEC-087. This resolves the "security.md SEC-087 vs status.md #321" inconsistency flagged in the 2026-07-12 system-gap audit (futures.md FUT-089).
>
> **Last updated:** 2026-07-12 — Security-reviewed the FUT-009 quick-win batch (PRs #328/#329/#330, `main...HEAD`). **1 new finding logged + RESOLVED: SEC-088 (MEDIUM), review label SEC-330-A.** #328 (show-password toggle) and #329 (actor display name) reviewed **CLEAN** — no findings (reveal only swaps the native input `type`, never persists/logs the secret; the audit display-name join is tenant-scoped via `LookupUsernames`→`WHERE u.tenant_id=$1`, additive proto field 10, fail-open only on a cosmetic label). **SEC-088 (MEDIUM) — FUT-009 free-form `signer_id` could forge service-account provenance:** the FUT-009 SA sign path records an SA's shadow user_id (a UUID) as `signer_id`, and the tag-detail Signing panel badges any UUID-shaped `signer_id` as a validated "Service account". The legacy free-form `signer_id` path accepted any printable string ≤256 chars — including a bare UUID — bypassing all SA existence/active/kind/tenant checks, so a repo-admin could POST a real SA's shadow UUID down the free-form path and forge managed-identity provenance (and potentially collide with that SA's later legitimate signature on the signer's `(digest, signer_id)` de-dupe). Applies in the supported single-tenant posture (not multi-tenant-dependent). **RESOLVED inline before merging #330** (`services/management/internal/handler/sign_manifest.go` `resolveSignerIdentity`): the free-form path now rejects a UUID-shaped value with `400 "signer_id must not be a UUID; use service_account_id"`, keeping the free-form and SA-shadow-UUID namespaces disjoint; non-UUID free-form identities unaffected (legacy FE-API-026 contract preserved). Regression tests: UUID free-form → 400 no-sign; non-UUID free-form → 201 signs. CWE-290. Verdict: **PASS** (no CRITICAL/HIGH; the one MEDIUM fixed pre-merge). **Previous entry:** Threat-modelled + security-reviewed **`feat/unified-artifact-catalog`** (9 commits, `main...HEAD`) — read-only additive metadata surfacing (`Repository.artifact_types`, `OrgSummary.image_repo_count`/`helm_repo_count`) through metadata → management BFF → FE. **1 new finding logged: SEC-087 (LOW).** No new auth flows, no mutations, no new user-supplied input (the `artifact_type` filter param is pre-existing). Verdict: **PASS — no merge blocker.** Tenant-isolation of the new artifact_types secondary query VERIFIED SAFE-BUT-IMPLICIT: `SELECT repo_id, config_media_type, media_type FROM manifests WHERE repo_id = ANY($1::uuid[])` is keyed only by repo_id, but the repo_id set is built exclusively from the immediately-preceding tenant-scoped `ListRepositories` result (`WHERE r.tenant_id = $1`, EXISTS subqueries also carry `m.tenant_id = r.tenant_id`), and `repositories.id` is a `gen_random_uuid()` global PK with `manifests.repo_id` an FK to it — so a repo_id from tenant A can only ever match manifests owned by tenant A. Not exploitable. Logged as **SEC-087 (LOW)** because it relies on upstream-filter correctness rather than being independently tenant-scoped, and CLAUDE.md §9 states metadata queries must filter by `tenant_id` unconditionally — trivial belt-and-suspenders fix (`AND tenant_id = $2`). SQL-injection VERIFIED SAFE: both new/changed queries are fully parameterised (`= ANY($N)` via pgx binding, no `fmt.Sprintf` in the SQL string; the pre-existing filterClause `fmt.Sprintf` only interpolates a `$N` placeholder index, never a value). Data-exposure VERIFIED: artifact_types + per-type repo counts are non-sensitive derived metadata; `config_media_type`/`media_type` never leave the server (classified in Go via `deriveArtifactType`, only the taxonomy label is emitted). Redirect VERIFIED SAFE: `/helm` → `throw redirect({ to: "/repositories" })` is a compile-time-constant internal target — no open-redirect surface. **Previous entry:** Threat-modelled + security-reviewed **FUT-023 Phase 1 ephemeral PR-scoped registries** (PR #293, `feat/fut-023-pr-registry-backend`, `main...HEAD`). **2 new findings logged: SEC-084 (LOW), SEC-085 (LOW).** The core trust boundary holds end-to-end: the unauthenticated `POST /webhooks/scm/github/pr` receiver caps the body with `MaxBytesReader`, forwards raw bytes verbatim, and delegates the *entire* trust decision to metadata's HMAC-SHA256 `Verify` (`prregistry/verify.go`), which is **constant-time** (`hmac.Equal`), **fail-closed** on every config gap (disabled / no secret / KEK unset → `ErrFeatureDisabled` → OUTCOME_DISABLED → bland 404 so the endpoint isn't a probe oracle), and maps a genuine mismatch → PermissionDenied → 401; neither the body nor the signature nor the secret is ever logged (only `X-GitHub-Delivery` for correlation). The webhook secret is write-only: sealed AES-256-GCM under `PR_REGISTRY_KEY_HEX`, never crosses the management↔metadata wire nor is echoed by Get/Put (masked to `has_secret`); a set-but-non-32-byte KEK fails closed at startup; a Put carrying a secret with no KEK → FailedPrecondition → 409. Teardown/promote are gated behind HMAC-verified events only, tenant-scoped (`TenantIDFromIncomingContext` → bootstrap tenant in single mode; empty → `uuid.Parse("")` → InvalidArgument in multi), idempotent, and audit-logged (`pr.namespace.provisioned` / `pr.namespace.torn_down` both mapped in the audit consumer). Org names are derived only via the HMAC path, only under the `pr-` prefix, and the `deriveOrgName` sanitiser provably satisfies `^[a-z0-9-]{2,64}$` (middle-truncation keeps prefix+suffix intact; well-tested). Promote-on-merge reuses `PromoteTag` — pure tenant-scoped DB manifest-digest copy, **no SSRF/path surface**, no org auto-creation. **No CRITICAL/HIGH/MEDIUM.** Both LOWs are hardening gaps, not exploitable paths: SEC-084 (`promote_target_org` not run through the org-name allowlist — parameterised downstream so no SQLi, fail-safe no-op on bad input) and SEC-085 (teardown adopts+cascade-deletes a pre-existing durable org on an exact `pr-<repo>-<N>` name collision; teardown org-DELETE also not tenant-scoped). Verdict: **PASS** — both findings recommended as fast-follows, neither a merge blocker. **Previous entry:** Security-reviewed the active-session-list feature on `feat/mfa-session-list` (PR #270, `main...HEAD`). **3 new findings logged: SEC-081 (MEDIUM), SEC-082 (LOW), SEC-083 (INFO).** Core session-auth properties all hold: `revoke:sid` gate is fail-CLOSED on Redis read error (mirrors `revoke:user`); ownership is enforced in SQL (`WHERE sid=$1 AND user_id=$2`, user_id always from validated JWT, never body/path); `sid` is preserved verbatim across `RefreshToken` while the JTI rotates and is empty for OCI/API-key/machine tokens; session routes are `requireAuth`-gated and `ValidateToken` rejects any typed (`mfa_setup`/`mfa_challenge`) token, so a partial/setup token cannot reach them; last_active updater is fire-and-forget on `context.Background()` and fail-OPEN; SQL is fully parameterised and the INET/UUID inputs are validated. **No CRITICAL/HIGH.** SEC-081 is the one substantive gap: the `revoke:sid` *write* is best-effort (copied from the SA-disable pattern) but — unlike the SA path — `ValidateToken` has no DB backstop, so a Redis write failure at revoke time silently no-ops the revocation while returning 204, and the session survives indefinitely via refresh. Verdict: **CONDITIONAL PASS** — recommend SEC-081 fixed inline before merge (surface the gate-set error so revoke isn't falsely reported successful). **Update 2026-07-05:** SEC-081 **RESOLVED** in the same PR — gate-set error now propagated (revoke returns 500 not a false 204) + idempotent `RevokeOwned`/`RevokeOthers` so the retry re-drives the gate; regression test added. SEC-082 (LOW) + SEC-083 (INFO) accepted as fast-follows. **Previous entry:** Threat-modelled + security-reviewed the TOTP MFA feature on `feat/mfa-totp` (`main...HEAD`, merge-base `b89919e`). **3 new findings logged: SEC-078 (MEDIUM), SEC-079 (MEDIUM), SEC-080 (LOW).** Core 2FA-bypass properties all hold (password alone never mints an access token; typed challenge/setup tokens rejected as access tokens; MFA/SSO KEK domains cryptographically separated; forced-enrolment authorizes only enroll/verify). No CRITICAL/HIGH. The two MEDIUMs are replay/brute-force robustness gaps in the second-factor path — recommend SEC-078 fixed inline before merge (one-line conditional UPDATE) and SEC-079 as a fast-follow. Verdict: **CONDITIONAL PASS**. **Update 2026-07-05:** all three RESOLVED — SEC-078 + SEC-080 shipped in PR #267 (commit `e1f9ee1`); SEC-079 shipped in PR #268 (`fix/sec-079-mfa-login-lockout` — lockout enforced at the MFA step + per-jti attempt cap). No open MFA findings remain.
>
...
**PENTEST-005 — Username enumeration via lockout/disabled status codes** — RESOLVED ✅ (scoped reversal 2026-07-05: login page now reveals lockout — see note below)
...
- **Scoped reversal (2026-07-05, product decision):** the **interactive `/api/v1/login` page** now surfaces the **locked** state — and only that state — as a distinct `423 Locked` with `code=ACCOUNT_LOCKED` + `retry_after_seconds` (so the operator sees "temporarily locked, try again in N minutes" instead of a confusing generic error while the correct password keeps "failing"). **Unchanged:** wrong-password and disabled-account failures still collapse to the generic `401`, and the `/auth/token` machine endpoint still collapses everything (locked included) to `401` — so the no-oracle guarantee holds everywhere except the deliberately-revealed lockout-after-N-failures case on the human login page. **Residual exposure:** an attacker who has already driven a specific username to lockout (≥5 failed attempts, which already requires guessing the username) can now confirm that username exists. Accepted for the **single-tenant self-hosted posture** (admin usernames are few and typically known; the lockout mechanism itself already permits targeted-account DoS regardless of the reveal). Implemented via `service.AccountLockedError{Until}` (unwraps to `ErrAccountLocked`, so every internal `errors.Is` check is unchanged); `errors.As` in the login handler produces the 423. Tests: `TestLogin_accountLocked_returns423WithRetry` + `TestLogin_wrongPassword_returns401Generic`.
...
### SEC-076 — goxmldsig XML-dsig signature bypass (GO-2026-4753) in the SAML SP dependency chain
- **Severity:** MEDIUM (dependency vulnerability, called path)
- **Status:** RESOLVED (2026-07-04, PR #256 — same day as detection)
- **Service:** `services/auth`
- **Raised:** 2026-07-04
...
### SEC-078 — TOTP replay guard is not atomic (check-then-act race lets one OTP be spent concurrently)
- **Severity:** MEDIUM
- **Status:** RESOLVED 2026-07-05 (PR #267, commit `e1f9ee1`) — `AdvanceMFACounter` is now an atomic conditional `UPDATE ... WHERE mfa_last_used_counter IS NULL OR < $1` returning `(bool, error)`; `verifyTOTP` treats a false (0-row) result as a replay. DB-level CAS-replay regression assertion added to `mfa_integration_test.go`.
- **Service:** `services/auth`
- **Raised:** 2026-07-05
- **Description:** The TOTP replay defence is a read-check-write with no serialization. `Service.ConsumeMFACode` / `CompleteMFAEnrollment` load `MFAState` once (`GetMFAState`), then `verifyTOTP` (`services/auth/internal/service/mfa.go:147-166`) checks `counter <= *st.LastUsedCounter` against that in-memory snapshot and, on pass, calls `AdvanceMFACounter`. The persist is an **unconditional** `UPDATE users SET mfa_last_used_counter = $1 WHERE id = $2` (`services/auth/internal/repository/mfa.go:85-88`) — no `WHERE mfa_last_used_counter < $1` guard, no row lock, no transaction spanning the read+write. Two concurrent requests presenting the **same valid OTP** both read the same (old) counter, both pass the strictly-increasing check, and both advance — so a single time-step code can be redeemed more than once in parallel. This defeats the "strictly-increasing counter / single-use TOTP" guarantee the migration comment and the feature threat model both claim. It affects every consumer of the code: two-step login (`VerifyLoginMFA` → two access tokens from one OTP), enrolment verify, and the disable/regenerate re-auth gate. Backup codes are NOT affected — they use a race-safe `UPDATE ... WHERE used_at IS NULL` + `RowsAffected` check (`MarkBackupCodeUsed`), which is exactly the pattern the TOTP counter path is missing. Exploitability requires an attacker able to submit a captured/observed valid OTP in parallel with (or racing) its legitimate use (e.g. a real-time phishing proxy, or replaying an intercepted `/login/mfa` body); the code's ~30-90s validity bounds the window.
- **Remediation:**
  1. Make the counter advance atomic and conditional: `UPDATE users SET mfa_last_used_counter = $1, updated_at = now() WHERE id = $2 AND (mfa_last_used_counter IS NULL OR mfa_last_used_counter < $1)`; treat `RowsAffected() == 0` as a replay and return `ok=false` (mirror `MarkBackupCodeUsed`'s race-loss handling).
  2. Have `verifyTOTP` report failure when the conditional advance reports zero rows affected, so the losing request of a concurrent pair is rejected rather than issued a token.
  3. Add a concurrency test that fires two goroutines with the same valid code and asserts exactly one succeeds.
- **References:** CLAUDE.md §7 (JWT/auth), feature threat-model item #2 (replay); CWE-367 (TOCTOU race condition); CWE-287 (Improper Authentication); OWASP ASVS V2.8 (OTP verifier single-use).

### SEC-079 — Second-factor brute-force is bounded only by per-IP rate limit; account lockout is fed but never enforced on the MFA step, and the challenge token is stateless/parallel-reusable
- **Severity:** MEDIUM
- **Status:** RESOLVED 2026-07-05 (branch `fix/sec-079-mfa-login-lockout`) — `VerifyLoginMFA` now (a) loads the user and rejects `ErrAccountLocked` when `LockedUntil` is in the future, before consuming the code; (b) `ResetFailedLogins` on success; (c) enforces a per-challenge-`jti` attempt cap (`maxMFAChallengeAttempts=5`) via an atomic Redis `Incr` keyed on the token's jti (TTL = challenge TTL, fail-closed on Redis error) so a stateless challenge cannot be replayed for unbounded guesses within its window. Three regression tests added (locked-account rejected, per-token cap bounds guesses even when the lockout write fails, success resets the counter).
- **Service:** `services/auth`
- **Raised:** 2026-07-05
- **Description:** `VerifyLoginMFA` (`services/auth/internal/service/auth.go:782-817`) records a failed attempt into the account-lockout counter (`RecordFailedLogin` + `LockUntil` at threshold) on a wrong OTP, but — unlike `AuthenticateUser` — it never **checks** `LockedUntil` before consuming a code and issuing a token. The `mfa_challenge` token is stateless: `issueTypedToken` records no `jti`, and `ValidateMFAToken` runs no revocation/attempt-count check, so a single challenge token can be replayed in unbounded parallel for its full 5-minute TTL. Consequently the only real bound on OTP guessing inside one challenge window is the per-IP limit in `loginMFA` (`CheckIPRateLimit`, ~10/min/IP). Against a 6-digit code with a ±1 skew window (3 codes valid at any instant), a distributed attacker who already holds the victim's password (MFA is the last line of defence) and reuses one challenge token across a botnet can reach ~5×10⁴ guesses per 5-min window → ≈15% chance of a successful MFA bypass per window. The in-code comment ("brute-forcing the second factor is bounded by the existing lockout policy") overstates the actual protection: the lockout only gates re-issuance of a *new* challenge token via the password path after the current one expires; it does nothing within the live window.
- **Remediation:**
  1. Enforce lockout on the MFA step: in `VerifyLoginMFA`, load the user and reject with `ErrAccountLocked` when `LockedUntil` is in the future (before `ConsumeMFACode`), and `ResetFailedLogins` on success. This caps guesses at `maxFailedLogins` (5) per 15-min lock cycle regardless of IP distribution.
  2. Make the challenge token single-window-bounded: record its `jti` and increment a per-`jti` attempt counter in Redis (TTL = challenge TTL), rejecting after a small N; invalidate the `jti` on the first successful verify so it cannot be reused to mint additional tokens.
  3. Keep the per-IP limit as defence-in-depth.
- **Residual (INFO, accepted):** the PR #268 fix implements remediation #2's per-`jti` attempt *cap* but descopes the "invalidate the `jti` on first successful verify" half — within the same TTL and remaining cap the challenge token is not burned on success, so it could in principle mint more than one access token. Not exploitable: each mint still requires a fresh single-use valid factor (TOTP counter advances per SEC-078; backup codes marked used), which an attacker lacking the secret cannot produce, so it fails safe. Two further INFO nits from the PR #268 review, both fail-safe: the `Expire` seeding is best-effort/non-atomic with `Incr` (a dropped TTL leaves the jti *capped*, not open); and the Redis-error path returns a wrapped error, but the `/login/mfa` handler collapses every `VerifyLoginMFA` error to a static `401 "invalid code"` (`http_mfa.go:70-77`), so no internal error string is echoed. Verified clean by a dedicated read-only security review 2026-07-05 — cleared for merge, no blockers.
- **References:** CLAUDE.md §7 (account lockout: 5 attempts → 15-min lock; IP rate limit 10/min), feature threat-model item #3 (brute force); CWE-307 (Improper Restriction of Excessive Authentication Attempts); CWE-799 (Improper Control of Interaction Frequency); OWASP ASVS V2.8.

### SEC-080 — Forced-enrolment verify issues the post-enrolment access token with `is_global_admin` taken from the (always-false) setup token instead of the fresh DB row
- **Severity:** LOW (fails safe — under-privileges, never over-privileges)
- **Status:** RESOLVED 2026-07-05 (PR #267, commit `e1f9ee1`) — both completion paths (login step-up + forced-enrolment verify) now route through a shared `IssueMFACompletedToken` that resolves roles + `is_global_admin` from the DB via `GetByID`, never from a caller-supplied claim. Service-level regression test added.
- **Service:** `services/auth`
- **Raised:** 2026-07-05
- **Description:** On the forced-enrolment path, `mfaVerify` mints the completing access token with `claims.IsGlobalAdmin` copied from the presented **setup** token (`services/auth/internal/handler/http_mfa.go:189-196`). Setup tokens are minted by `issueTypedToken`, which never sets `IsGlobalAdmin` (always `false`), so a genuine global admin forced to enrol receives a first access token with `is_global_admin=false` until their next login/refresh. This is a correctness inconsistency, not a privilege-escalation: the two-step-login path (`VerifyLoginMFA`) deliberately re-loads the user via `GetByID` to get the fresh flag, whereas the setup-verify handler does not. Direction of failure is safe (a global admin is briefly under-privileged), so it is not exploitable, but it is a latent surprise and diverges from the login path's contract.
- **Remediation:**
  1. In `mfaVerify`'s `isSetup` branch, resolve `is_global_admin` and `principal_kind` from the DB (add a service helper or reuse the `VerifyLoginMFA` reload) instead of trusting the typed-token claim, so the post-enrolment token matches what a fresh login would issue.
  2. Alternatively, document that the first post-forced-enrolment token intentionally omits admin context and relies on refresh — but re-loading is cleaner and matches the login path.
- **References:** CLAUDE.md §7 (JWT claims), REDESIGN-001 Phase 5.1 (`is_global_admin`); CWE-708 (Incorrect Ownership Assignment) / general least-privilege consistency.

### SEC-081 — `revoke:sid` gate write is best-effort with no DB backstop on the validate path: a Redis write failure at revoke time silently no-ops session revocation while reporting 204 success
- **Severity:** MEDIUM (approaches HIGH — it degrades the exact "kill my stolen session" security function — but is conditioned on a Redis write failure at revoke time)
- **Status:** RESOLVED (PR #270, `feat/mfa-session-list`)
- **Service:** `services/auth`
- **Raised:** 2026-07-05
- **Resolved:** 2026-07-05 — Remediation #1 applied (the recommended minimal fix). `setSessionRevokeGate` now returns its Redis `Set` error (with an added `s.redis == nil` no-op guard); `RevokeSession` and `RevokeOtherSessions` propagate it, so a failed gate write returns `500` instead of a false `204` — the revocation is never silently reported as done. To make the client's retry actually re-drive the gate (rather than 404 on an already-DB-revoked row), `RevokeOwned` and `RevokeOthers` (`repository/session.go`) were made **idempotent**: `revoked_at = COALESCE(revoked_at, now())` with the `revoked_at IS NULL` filter replaced by `expires_at > now()`, so an owned unexpired row still RETURNs its `expires_at` on retry. Note the failure surface is narrow in practice: a *full* Redis outage makes `ValidateToken`'s `revoke:sid` read fail **closed** (denies all session tokens), so this gap only bites on an *asymmetric/transient* write failure — for which the retry path now closes it. Regression test `TestRevokeSession_gateSetFailure_returnsError` (closes miniredis, asserts `err != nil && !ok`). Remediation #2/#3 (hot-path DB backstop / refresh-time row re-check) intentionally not taken — the extra per-validation DB read is not justified given the fail-closed-on-outage behaviour + idempotent retry.
- **Description:** `RevokeSession` / `RevokeOtherSessions` mark `user_sessions.revoked_at` in Postgres (authoritative) and then call `setSessionRevokeGate` (`services/auth/internal/service/session.go:116-124`) to write the `revoke:sid:<sid>` Redis key that `ValidateToken` actually enforces. `setSessionRevokeGate` **logs and swallows** any Redis `Set` error and returns `void`; `RevokeSession` returns `true` (→ handler `204 No Content`) regardless. The problem is that the enforcement path has **no DB fallback**: `ValidateToken` (`auth.go:517-529`) consults **only** the `revoke:sid` Redis key for session revocation — it never re-reads `user_sessions.revoked_at`. This diverges from the pattern this PR copied: the service-account disable path (`service_account.go:372-384`) is *also* best-effort on the Redis `Set`, but there the DB (`disabled_at`) is re-checked on every `ValidateAPIKey` HIT, so a dropped Redis write fails safe. Sessions have no such backstop. Consequently, if Redis is unavailable (or the single key write fails) at the instant a user revokes, the DB row is flipped to revoked, the session vanishes from `ListLive` (which filters `revoked_at IS NULL`) so the UI shows success, yet the outstanding JWT keeps validating — and because `RefreshToken` → `ValidateToken` also only checks Redis, the attacker can refresh the session **indefinitely** past the 300s JTI TTL. The user is told the session is dead; it is not.
- **Remediation:**
  1. Make `setSessionRevokeGate` return its Redis error and have `RevokeSession` / `RevokeOtherSessions` propagate it, so `revokeSession` / `revokeOtherSessions` return `500` and the caller retries instead of being told the revoke succeeded. (Minimal, matches the "revocation must not be silently reported as done" invariant.)
  2. Optionally add a DB backstop mirroring the SA path: when `claims.Sid != ""` and no `revoke:sid` key is present, treat a *missing/erroring* gate as authoritative-unknown and (cheaply, e.g. behind the same Redis-negative-cache) verify `user_sessions.revoked_at IS NULL`. Weigh against the extra hot-path DB read.
  3. Have `RefreshToken` refuse to carry a `sid` forward once its backing row is revoked/absent, so refresh can never resurrect a killed session even if the live-token check is bypassed.
- **References:** CLAUDE.md §7 (JTI revocation fail-closed; principal-revocation posture), SEC-005 (revoke-gate TTL coupling — correctly implemented here), the SA-disable precedent in `service_account.go:372-384`; CWE-613 (Insufficient Session Expiration), CWE-284 (Improper Access Control); OWASP ASVS V3.3 (session termination).

### SEC-082 — last_active debouncer fails OPEN into an unbounded per-validation goroutine + DB write, amplifying a Redis outage into an auth-DB write storm on the hot validation path
- **Severity:** LOW (telemetry-only, self-inflicted under a Redis outage, fails safe for auth correctness)
- **Status:** OPEN
- **Service:** `services/auth`
- **Raised:** 2026-07-05
- **Description:** `ValidateToken` fires `sessionActive.Touch(context.Background(), sid)` for every session-bearing token (`auth.go:537-540`), which spawns a goroutine (`session.go:181`). `touchNow` (`session.go:187-201`) debounces via Redis `SETNX EX 60s`, but is deliberately fail-OPEN: on any Redis error it falls through to the DB `UPDATE`. Because `ValidateToken` is the gRPC path every service calls to validate a Bearer token, a Redis outage removes the debounce entirely, so *every* validation of a session token becomes a fresh goroutine performing a `user_sessions` `UPDATE` — turning a cache degradation into a write amplification against the same auth Postgres that is already under stress during the outage. `context.Background()` also means these writes carry no deadline and cannot be shed by request cancellation. This is the correct *direction* for a telemetry field (never blocks or denies auth), but the fail-open amplification is unbounded.
- **Remediation:**
  1. On a Redis error, skip the write (fail-open should mean "skip the debounced telemetry", not "do the expensive thing unconditionally") — or gate the fallback write behind a coarse process-local rate limiter / `singleflight` per sid so a Redis outage cannot fan out one DB write per validation.
  2. Give `touchNow` a short bounded context (e.g. `context.WithTimeout(context.Background(), 2s)`) so a slow/stalled DB cannot accumulate unbounded goroutines.
  3. Consider a bounded worker/channel instead of `go` per validation to cap concurrency.
- **References:** CLAUDE.md §6 (gRPC deadlines), FUT-003 lastUsedUpdater precedent (same fail-open idiom, same latent amplification); CWE-770 (Allocation of Resources Without Limits or Throttling).

### SEC-083 — session sweeper idle window is hardcoded to 14d and can delete rows the tenant idle policy still considers live
- **Severity:** INFO (fails safe — deletes early → logs out sooner; not a weakening; single-tenant posture)
- **Status:** OPEN
- **Service:** `services/auth`
- **Raised:** 2026-07-05
- **Description:** `ListSessions` computes its idle cutoff via `idleCutoff` (`session.go:128-139`), which honours the tenant token policy's `idle_revoke_days` when configured (overriding the 14d default). The background sweeper, however, is wired with a **fixed** `14*24*time.Hour` idle window (`server.go` `NewSessionSweeper(sessionRepo, 14*24*time.Hour, …)`) and `DeleteExpired` hard-deletes any row with `last_active_at < now-14d`. If an operator sets `idle_revoke_days > 14`, the sweeper physically deletes session rows that `ListLive` would still return, so a session silently disappears from the list before its policy idle window elapses. The failure direction is safe (earlier logout, never a longer-lived session), and the platform is single-tenant with the default policy, so impact is minimal — logging it for correctness/consistency.
- **Remediation:**
  1. Derive the sweeper's idle window from the same policy source `idleCutoff` uses (or set it to the maximum configurable idle window) so GC never outpaces the list's liveness rule.
  2. Alternatively, document that the sweeper enforces a hard 14d ceiling on idle sessions regardless of policy.
- **References:** consistency with `idleCutoff` / tenant token policy; CWE-613 (session-lifetime consistency, benign direction).

### SEC-084 — `promote_target_org` accepted without org-name allowlist validation
- **Severity:** LOW
- **Status:** OPEN
- **Service:** `services/management` + `services/metadata`
- **Raised:** 2026-07-10
- **Description:** FUT-023 PR-registry config lets an admin set `promote_target_org` (the durable org a merged PR's images are copied into). Neither the BFF (`services/management/internal/handler/pr_registry.go:305`, `handlePutPRRegistryConfig`) nor the metadata handler (`services/metadata/internal/handler/grpc_pr_registry.go:115`, `PutPRRegistryConfig`) validates this string against the platform org-name allowlist `^[a-z0-9-]{2,64}$` (CLAUDE.md §7), unlike `handleCreateRepository` which validates `org` before any gRPC call. The value flows unvalidated into `promoteNamespace` → `PromoteTag` as `DstOrg`, where it is used only in parameterised `o.name || '/' || r.name = $2` comparisons — so there is **no SQL-injection exposure** (pgx parameterisation holds), and the blast radius is limited to an admin-privileged config field. The concrete effect of a malformed value is a silent no-op promote (no org matches, so `PromoteTag` returns `ErrNotFound` and the merge aborts before teardown, leaving the namespace intact for retry — fail-safe). Logged for §7 input-validation-at-the-edge compliance rather than an exploitable path.
- **Remediation:**
  1. Validate `promote_target_org` against the org-name regex in `handlePutPRRegistryConfig` (reuse `validateOrgName`) and reject with 400 before forwarding, mirroring `handleCreateRepository`.
  2. Optionally re-validate in the metadata `PutPRRegistryConfig` handler as defence-in-depth (the gRPC server should not trust the BFF blindly).
- **References:** CLAUDE.md §7 (input validation — allowlist before downstream); CWE-20.

### SEC-085 — PR teardown adopts + cascade-deletes a pre-existing durable org whose name collides with the derived `pr-<repo>-<N>` namespace
- **Severity:** LOW (single-tenant posture; requires an operator-created org matching the `pr-` naming scheme)
- **Status:** RESOLVED (2026-07-10, `fix/sec-085-org-adoption-guard`) — remediation #2 (adoption guard) + #3 (tenant-scoped teardown DELETE) shipped; #1 not needed (the #2 guard makes the created-vs-adopted flag redundant).
- **Service:** `services/metadata`
- **Raised:** 2026-07-10
- **Description:** On an `opened`/`reopened` PR, `provision` (`services/metadata/internal/prregistry/service.go:141`) calls `GetOrCreateOrganization(tenantID, orgName)` where `orgName = "pr-<sanitized-repo>-<N>"`. `GetOrCreateOrganization` (`repository.go:532`) is `INSERT ... ON CONFLICT (tenant_id, name) DO UPDATE ... RETURNING id` — so if an org with that exact name **already exists** (e.g. an operator legitimately named an org `pr-myapp-42`), provision silently **adopts** the existing org id into the `pr_namespaces` lifecycle row instead of minting a fresh ephemeral one. When the PR later closes/merges, `tearDown` → `TearDownPRNamespace` (`repository.go:204`) hard-`DELETE`s that org by id (`DELETE FROM organizations WHERE id = $1`), destroying the operator's pre-existing org and all its repositories via cascade. The HMAC gate means only a legitimately-signed GitHub delivery can trigger this, and the `pr-` prefix + PR-number suffix make an accidental collision unlikely, but the teardown DELETE trusts that the stored `org_id` is genuinely ephemeral without recording/checking that provision *created* it (vs. adopted it). Note also `TearDownPRNamespace`'s org DELETE is **not tenant-scoped** in its WHERE clause (`DeleteOrganization` is, but the teardown path is keyed by org_id only) — benign today because org_id is a tenant-derived PK, but inconsistent with the §9 belt-and-suspenders posture the same file documents.
- **Remediation:**
  1. Track whether provision *created* the org (e.g. use `INSERT ... ON CONFLICT DO NOTHING` + detect the created-vs-existing case, or persist a `created_org` boolean on `pr_namespaces`) and skip the org DELETE at teardown when the org was adopted, not minted.
  2. Alternatively, guard against adoption at provision time: refuse to provision when an org with the derived name already exists that is not itself a torn-down PR org.
  3. Add `tenant_id = $2` to the `TearDownPRNamespace` org-DELETE WHERE clause to match `DeleteOrganization` and the §9 posture.
- **References:** CLAUDE.md §9 (tenant isolation / scoped deletes); CWE-284 (improper access control on a destructive op); CWE-708 (incorrect ownership assumption).
- **Resolution:** `provision` (`services/metadata/internal/prregistry/service.go`) now runs an adoption guard **before** `GetOrCreateOrganization`: it calls `LookupOrgIDByName(tenant, orgName)` (added to the `Store` interface, backed by the existing `Repository.LookupOrgIDByName`) and, when an org with the derived `pr-<repo>-<N>` name already exists, only proceeds if that org is the exact one our own **active** `pr_namespaces` row already points at (a GitHub re-delivery). Any other pre-existing org is a collision with an operator-owned org — provision refuses (logged `slog.Warn`, returns `OutcomeIgnored` with no error, so the foreign org is left untouched and no GitHub redelivery storm is triggered). Because provision can no longer *adopt* a foreign org, teardown can never cascade-delete one. Remediation #3 (tenant-scoped org DELETE, `WHERE id = $1 AND tenant_id = $2` in `TearDownPRNamespace`) shipped with PR A. Regression tests: `TestHandleEvent_ForeignOrgCollisionRefused` (foreign collision → refused, no upsert/publish) + `TestHandleEvent_OwnOrgReprovisionAllowed` (own re-delivery → provisioned).

### SEC-086 — SCIM `POST /scim/v2/Users` provisioned IdP attributes without §7 allowlist validation
- **Severity:** MEDIUM (superuser provisioning surface; IdP is an external, only-partly-trusted input source)
- **Status:** RESOLVED (2026-07-11, `feat/scim-provisioning` / PR #319) — validation added at the SCIM edge before `Provision`.
- **Service:** `services/auth`
- **Raised:** 2026-07-11
- **Description:** The SCIM 2.0 create handler `scimCreateUser` (`services/auth/internal/handler/http_scim.go`) forwarded the IdP-supplied `userName`, primary `email`, `externalId`, and `displayName` straight into `Service.Provision` without validating them against the CLAUDE.md §7 allowlists (`^[a-zA-Z0-9_-]{3,64}$` username, a real email check) or bounding the free-form TEXT attributes. Although pgx parameterisation removes any SQL-injection exposure and the SCIM principal carries no RBAC roles, an IdP (or a caller holding the global SCIM token spoofing one) could smuggle a `userName` containing characters the rest of the platform assumes usernames never contain, or push an unbounded `externalId`/`displayName` into the DB. This is the classic §7 "validate untrusted input at the edge" gap on a directory-provisioning path that mints platform users.
- **Remediation:**
  1. Add `validateSCIMCreate(userName, email, externalID, displayName)` in the handler (`scim_types.go`) enforcing the §7 username regex, an email sanity check, and a 255-byte cap on `externalId`/`displayName` (320-byte cap on `email`, RFC 5321), returning 400 `scimType=invalidValue` on the first failure **before** `Provision` is called. Empty `userName`/`email` remain allowed individually because the create path already requires at least one and derives a safe username from the email.
- **Resolution:** Shipped in PR #319 alongside the SCIM Users-only v1 review-batch fold. Regression test `TestSCIMCreate_rejectsInvalidAttributes` covers bad userName (space + over-length), malformed email, over-long externalId/displayName (each → 400, zero users provisioned) plus a valid-create sanity case (→ 201) guarding against over-strict rules.
- **References:** CLAUDE.md §7 (input validation — allowlist at the handler layer); CWE-20 (improper input validation).

### SEC-087 — `ListRepositories` artifact-types secondary query is not independently tenant-scoped
- **Severity:** LOW (defense-in-depth; not exploitable — see analysis)
- **Status:** RESOLVED (2026-07-13)
- **Service:** `services/metadata`
- **Raised:** 2026-07-12
- **Resolution:** Fixed **inline in PR #321** (unified-artifact-catalog) — the secondary query now threads `tenantID` as `$1`: `SELECT repo_id, COALESCE(config_media_type, ''), media_type FROM manifests WHERE tenant_id = $1 AND repo_id = ANY($2::uuid[])` (`services/metadata/internal/repository/repository.go:289-291`, with an explanatory `// SEC-087` comment). Verified in-code 2026-07-13 during a tracker truth-up — the status.md #321 row already recorded the fix while this entry was stale at OPEN. Remediation #2 (a cross-tenant regression test) remains an accepted non-blocking follow-up (the fix is a one-line tenant filter on a non-exploitable path; the `artifact_type_test.go` unit suite covers the derivation logic).
- **Description:** In the unified-artifact-catalog feature, `Repository.ListRepositories` runs a second-pass query to attach per-repo artifact types: `SELECT repo_id, COALESCE(config_media_type, ''), media_type FROM manifests WHERE repo_id = ANY($1::uuid[])` (`services/metadata/internal/repository/repository.go:284-286`). Unlike every other metadata read, this query is keyed **only** by `repo_id` and omits an explicit `tenant_id` filter — contravening CLAUDE.md §9 ("All queries in `registry-metadata` must filter by `tenant_id` — never query across tenants"), stated as an unconditional rule. **This is NOT an exploitable cross-tenant leak today:** the `repoIDs` slice is built exclusively from the immediately-preceding tenant-scoped `ListRepositories` result (`WHERE r.tenant_id = $1`; its `artifact_type` filter EXISTS subqueries also carry `m.tenant_id = r.tenant_id`), `repositories.id` is a `gen_random_uuid()` global PRIMARY KEY (`migrations/00001_initial_schema.sql:17`), and `manifests.repo_id` is an FK to it (`:30`), so a `repo_id` drawn from tenant A's result set can only match manifests owned by tenant A. The risk is purely that correctness depends on the upstream filter staying correct; a future refactor that widened how `repoIDs` is populated (e.g. a caller passing IDs from another source) would silently become a cross-tenant read with no second barrier.
- **Remediation:**
...
- **Remediation (SHOULD-FIX follow-ups, non-blocking):**
  1. **SF-1 (LOW) — FALSE POSITIVE, then hardened (2026-07-14):** the review inferred management error responses lacked `X-Content-Type-Options: nosniff`. On investigation this was **already covered** — the management CORS middleware (`services/management/internal/middleware/cors.go`) set `nosniff` + `X-Frame-Options: DENY` unconditionally on every response, verified live on both 200 and 401 (`curl -D -` against the running BFF). The genuine (marginal) gaps were that management used bespoke inline header-setting instead of the shared `libs/middleware/http.SecureHeaders` every other service uses, and lacked `X-XSS-Protection: 0`. **Resolved:** `services/management/internal/server/buildHandler` now wraps the chain with the shared `httpmiddleware.SecureHeaders` as the outermost middleware (covers CORS preflight 204s + auth 401s), the duplicate headers were removed from `cors.go`, and `X-XSS-Protection: 0` now lands too — completing §17 and matching the platform pattern. Regression test `server.TestBuildHandler_SetsSecurityHeaders` asserts all three headers on a success response.
...
### SEC-090 — Signing coverage rollup BFF endpoint reviewed (`feat/signing-coverage-rollup`)
- **Severity:** INFO
- **Status:** RESOLVED (2026-07-16, reviewed PASS)
- **Service:** `services/management` + `frontend/`
- **Raised:** 2026-07-16
- **Description:** Threat-modelled the new read-only workspace signing-coverage rollup endpoint `GET /api/v1/signing/coverage?window=` (`services/management/internal/handler/signing_coverage.go`, wired reader-allowed at `handler.go:474`) plus the Security → Signing coverage FE tab (`frontend/src/lib/api/signing-coverage.ts`, `frontend/src/components/security/signing-coverage-*.tsx`, route `frontend/src/routes/_authenticated.security.signing.tsx`). Pure gRPC orchestration (`ListRepositories` → per-repo `ListTags` + signer `ListSignatures` + `ListRepositoryTrustedKeys`); no proto change, no DB migration, no new secret, no mutation. Threat model verdict **PASS** — 0 CRITICAL / 0 HIGH / 0 MEDIUM / 0 LOW / 2 INFO (both accepted, non-blocking). Findings by area:
  1. **AuthZ / access bar — ACCEPTABLE, no widening.** Reader-allowed is correct and precedented. The access bar matches two established siblings: (a) `handleSecurityOverview` (FE-API-020, `security.go:55-59`) is explicitly "any authenticated tenant user — no extra RBAC scope check beyond tenant membership"; (b) `handleListTrustedKeys` / `handleListRecentSigners` (`trusted_keys.go`) are reader-allowed because key_ids/signer_ids are public identifiers already visible on tag-detail pages. Critically, the pre-existing reader-allowed `handleListRepositories` (`handler.go:908`) already returns the ENTIRE tenant repo inventory to any authenticated user (filters on `tenant_id` only, never per-repo RBAC grants), so the workspace-wide fan-out here discloses signing *posture* over an inventory a reader can already enumerate. No field returned (per-repo signed-tag %, key_ids, signer_ids, trusted-key allowlist state, allowlist_health) exceeds what a reader already reaches via `/repositories`, `/security/overview`, and per-repo `/trusted-keys` + `/recent-signers`. The endpoint does not raise the effective disclosure surface.
  2. **Tenant isolation — VERIFIED CORRECT.** `tenantID` is server-derived from the validated JWT (`resp.GetTenantId()`) or validated API key (`kr.GetTenantId()`) in `middleware.RequireAuth` (`middleware/auth.go:82,112`) and read via `middleware.TenantIDFromContext` (`signing_coverage.go:121`); it is never sourced from a request header or body. The same `tenantID` is threaded into every one of the four downstream gRPC calls (`listAllReposForCoverage`, `recentTagDigests`, `coverageSignatures`, `coverageTrustedKeys`). No `repo_id` reaches signer/metadata that wasn't produced by the immediately-preceding tenant-scoped `ListRepositories`. Single-tenant platform (ADR-0031), but the plumbing is correct regardless.
  3. **Input validation — VERIFIED SAFE.** Sole user input is `?window=`, parsed via `strconv.Atoi`, rejected `<1` with 400, capped at `maxCoverageWindow=200` (`signing_coverage.go:123-134`). No string reaches SQL/shell/storage-key construction. Resource-abuse VERIFIED BOUNDED: repo enumeration capped at `maxReposPerRollup=1000`; fan-out bounded to `coverageFanoutWorkers=8` via a buffered semaphore; each signer call carries a `coverageSignerTimeout=3s` deadline; a 60s process-wide TTL cache absorbs repeated dashboard loads. `window` cannot amplify fan-out beyond the per-repo `ListTags` page size (itself capped at 200). No unbounded fan-out or DoS lever.
  4. **Info disclosure / logging — ACCEPTABLE.** Warn-level logs on downstream errors carry repo label (`org/name`), manifest digest, and key_id (`signing_coverage.go:327,337,374,386`). Per the platform "never log secrets" rule these are all non-secret: digests are content addresses, key_ids are public-key identifiers (same class the reader-allowed trusted-keys/recent-signers routes already emit in responses), org/repo labels are non-sensitive. No token, private-key material, or request body is logged. User-facing error bodies are generic (`"failed to list repositories"`), no gRPC status code / stack trace / internal service name leaked (matches §7 management-API rule).
  5. **Fail-open posture — ACCEPTABLE for a visibility endpoint.** Every downstream error degrades the affected field and still returns 200 (per-repo helpers return nil/empty on error; `signer==nil` short-circuits to `signer_enabled:false`). This is correct: the endpoint changes NO admission decision (it is deliberately separate from the deferred signed-image-admission enforcement work, per the file header), so fail-open here degrades visibility only — it can never weaken an enforcement gate. Contrast: a fail-CLOSED design would blank the whole rollup on one flaky repo, harming the operator without any security gain. Consistent with the sibling `handleListRecentSigners` posture.
  6. **Cache — VERIFIED NO POISONING / NO BLEED.** Key is `tenantID + "|" + strconv.Itoa(window)` (`signing_coverage.go:149`) where `tenantID` is the server-derived value from #2 — an attacker cannot influence the tenant component, so no cross-tenant key collision is constructible. The `signer==nil` degraded response is returned BEFORE the cache read/write (`:139-147`), so a disabled-signer body is never served from, or written into, an enabled-signer cache slot. Values are computed server-side from tenant-scoped data; there is no attacker-controlled path into a cached body.
- **INFO follow-ups (accepted, non-blocking):**
  1. **SEC-090-A (INFO) — TTL cache never evicts expired entries.** `coverageTTLCache.get` treats an expired entry as a miss but does not delete it; `set` only overwrites the same key (`signing_coverage.go:98-112`). Unbounded in theory, but bounded in practice: keys are `(tenant_id, window)` with `window ∈ [1,200]` and a single bootstrap tenant, so the map tops out at ~200 entries. Not a leak on this platform. Optional hardening: drop the stale entry on read, or reap on `set`.
  2. **SEC-090-B (INFO) — no per-repo authorization filter.** By design the rollup shows every tenant repo's posture to any reader, matching `/repositories` and `/security/overview`. If the platform ever introduces per-repo read-scoping on those endpoints, this rollup must be updated in lockstep (it inherits, and must keep inheriting, the same visibility model). Documented so a future RBAC-tightening pass does not overlook it.
- **References:** CLAUDE.md §7 (input validation, management-API generic errors), §9 (tenant isolation), §17 (hardening checklist); futures.md "Signing coverage rollup"; sibling precedent FE-API-020 (`security.go`) + Tier 1 #3 trusted-keys/recent-signers (`trusted_keys.go`); CWE-639 / CWE-284 / CWE-770 (all verified NOT present).
