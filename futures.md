...
- **⚠️ Deferred fast-follow (SEC-085 — org-adoption guard):** provision currently upserts the ephemeral org via `GetOrCreateOrganization`, so a `pr-<repo>-<N>` name that collides with a pre-existing operator org silently *adopts* it — and teardown would then cascade-delete that operator org. The fast-follow is to guard against adoption at provision time (either refuse to provision onto an already-existing org, or track a created-vs-adopted flag so teardown only deletes orgs it created). Not implemented in PR A; provision logic is unchanged. The teardown mark-UPDATE + org-DELETE are now tenant-scoped (SEC-085 #3, PR #293 review) so the blast radius is confined to the tenant, but the same-tenant name-collision adoption case remains open.
