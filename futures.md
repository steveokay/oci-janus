...
### FUT-018 — Digest-keyed scan + signature BFF routes (FUT-017 Phase 2b)

**Surfaced:** 2026-06-24 during FUT-017 Phase 2 build. The Phase 2
agent flagged that the existing `useScan` / `useSignature` FE hooks
are **tag-keyed** (`/api/v1/repositories/{org}/{repo}/tags/{tag}/scan`)
even though the underlying scanner + signer gRPC services already
accept arbitrary manifest digests internally. Cached proxy rows
carry only `digest`, not `org/repo/tag`, so building the FUT-017
Scans + Signing tabs against the existing routes is structurally
impossible.

**Scope:**

New BFF routes on `services/management` (workspace-admin gated):

- `GET /api/v1/scan-by-digest/{digest}` — wraps
  `scanner.GetScanStatus(manifest_digest)` (already exists; the RPC
  takes a digest). Returns the same shape the per-tag scan route does.
- `GET /api/v1/signatures-by-digest/{digest}` — wraps
  `signer.ListSignatures(manifest_digest)` (already exists). Returns
  the same shape the per-tag signature route does.
- (Optional Phase 2b.1) `POST /api/v1/sign-by-digest/{digest}` —
  wraps `signer.SignManifest`. Lets the operator manually sign a
  cached manifest without setting up auto-sign policy.

Validation: digest matches `^sha256:[a-f0-9]{64}$`.

**Frontend changes (Phase 2b.2):**

- New FE hooks `useScanByDigest(digest)` and
  `useSignaturesByDigest(digest)` mirroring the existing
  tag-keyed ones.
- **Scans tab** on `/workspace/proxy-cache/{id}` — reuses the
  existing `<ScanPanel>` shape; just pointed at the digest hook.
- **Signing tab** on same page — reuses the existing
  `<SignatureList>` + `<SignManifestDialog>` shape.
- **Severity column** on the cache list table (badge from worst
  severity in scan results; em-dash when no scan).
- **Signed column** on the cache list table (check icon when
  >=1 signature; em-dash when zero).

**Why this matters for FUT-017:** Phase 2a shipped the per-upstream
policy editor (PR #89). Operators can flip auto-scan / auto-sign on
and the backend consumers do queue scans + sign manifests. But the
UI doesn't yet show those results. FUT-018 closes that loop.

**Affects:** `services/management` (new BFF routes), `frontend/`
(new hooks + tab + 2 columns).

**Effort:** ~1-2 days. Backend half is ~half day (3 thin REST
wrappers; the RPCs already exist). Frontend half is ~1 day (4
new tab + column components, but each is a near-mirror of an
existing one). Tests + smoke verification on top.
