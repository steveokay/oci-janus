...
| 1f — Theme toggle (light / dark / system) + Cmd+K palette | ✅ | `themeStore` toggles a `.dark` class on `<html>` from `localStorage` keyed `janus.theme`; tracks `prefers-color-scheme` for system mode. `ThemeToggle` segmented control sits inside `UserMenu`. Cmd+K palette built on `cmdk` + Radix Dialog, mounted at the AppShell level via `useCommandPaletteShortcut`. Three groups (Pages, Repositories, Actions) plus theme + sign-out commands. |
...
| **Honesty pass (post-1f)** | Removed the demo `ActivityFeed`, `TopRepos`, `DemoBanner`, hardcoded notification bell with `2` badge, and the misleading sparklines on live (Repositories + Storage) tiles. Demo tiles (Tags + Scans today) now carry a small "Demo" pill so the placeholder data is visible without the page-level yellow strip. |
| **Pinned repositories** | `pinnedReposStore` + `lastVisitedRepoStore` (both `localStorage`-backed Zustand stores). Pin/unpin toggle in the detail-page hero; `PinnedRepos` panel replaces the previous demo `TopRepos` on the dashboard. Empty state walks the user through pinning from any repo detail page. |
| **Dark-mode hero fix** | Every photographic hero (dashboard, /repositories, repo detail, /profile) and the warm-gradient empty states wrap their light layers in `dark:hidden` and swap to a single subtle indigo gradient under `.dark`. No more white veil on a dark page. |
| **Shared state components** | `EmptyPanel`, `ErrorPanel`, `TableSkeleton` extracted into `components/ui/states/`. Detail page, repos list, profile, and the page-level fallbacks all rebuilt off these. |
| **Visual polish bundle (10 items)** | Foundation: `Avatar` (12 deterministic OKLCH gradient pairs hashed from `org/repo`), `AnimatedNumber` (`framer-motion` count-up tween with `prefers-reduced-motion` guard), `lib/motion.ts` (shared variants for fade / list stagger / dialog spring), six hand-rolled SVG illustrations in `components/ui/illustrations.tsx`, and `bg-textured` SVG-noise utility. Applied across: stat tiles (`text-display-lg` numbers + count-up + tone-tinted accent line + `group-hover:scale-105` icon chip), hero (weather-style icon by period + contextual-tip waterfall: last-visited link → vulns → empty-workspace → all-clear), sidebar (3px primary accent bar on active nav), tables (gradient avatars + `opacity-0 group-hover/row:opacity-100` action reveal + storage tone-bar with utilisation %), page transitions (180ms route fade via `motion.div key={pathname}`), and a new `Footer` with a live system-health dot + version + docs links + a Radix-Dialog keyboard-shortcuts cheat-sheet. |
