...
func TestBuildGRPCOptions_NilExtraUnary(t *testing.T) {
	opts, err := buildGRPCOptions(&config.Config{}, nil, nil)
	if err != nil {
...
func TestBuildGRPCOptions_WithExtraUnary(t *testing.T) {
	...
	withNil, err := buildGRPCOptions(&config.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("buildGRPCOptions(nil): %v", err)
	}
	withExtra, err := buildGRPCOptions(&config.Config{}, nil, noop)
	if err != nil {
...
func TestBuildGRPCOptions_BadMTLSPaths(t *testing.T) {
	...
	if _, err := buildGRPCOptions(cfg, nil, nil); err == nil {
