// RED-FU-011 — smoke tests for buildGRPCServerOptions, the Phase 3.4 helper
...
func TestBuildGRPCOptions_NilExtraUnary(t *testing.T) {
	opts, err := buildGRPCServerOptions(&config.Config{}, nil)
	if err != nil {
		t.Fatalf("buildGRPCServerOptions(nil): unexpected err: %v", err)
	}
...
func TestBuildGRPCOptions_WithExtraUnary(t *testing.T) {
	...
	withNil, err := buildGRPCServerOptions(&config.Config{}, nil)
	if err != nil {
		t.Fatalf("buildGRPCServerOptions(nil): %v", err)
	}
	withExtra, err := buildGRPCServerOptions(&config.Config{}, noop)
	if err != nil {
		t.Fatalf("buildGRPCServerOptions(extra): %v", err)
	}
...
func TestBuildGRPCOptions_BadMTLSPaths(t *testing.T) {
	...
	if _, err := buildGRPCServerOptions(cfg, nil); err == nil {
		t.Fatal("buildGRPCServerOptions with bogus mTLS paths must return an error, got nil")
