...
// TestBootstrap_DifferentTenant_Fails verifies that running a second bootstrap
// with a DIFFERENT tenant ID is refused with a *ValidationError — the platform
// is single-tenant (ADR-0031), so a deployment owns exactly one tenant.
func TestBootstrap_DifferentTenant_Fails(t *testing.T) {
	ctx := context.Background()
	authDSN, tenantDSN := setupDatabases(t)
