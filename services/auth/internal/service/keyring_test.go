...
func TestValidateToken_SignAValidateAB(t *testing.T) {
	...
	token, err := svcSigner.IssueToken(ctx, uuid.New().String(), uuid.New().String(), nil, nil, false, "human")
	if err != nil {
...
func TestValidateToken_SignAValidateBOnly_fails(t *testing.T) {
	...
	token, err := svcA.IssueToken(ctx, uuid.New().String(), uuid.New().String(), nil, nil, false, "human")
	if err != nil {
		t.Fatalf("IssueToken: %v", err)
	}
...
func TestSingleKeyRingFromB64_roundTrip(t *testing.T) {
	...
	token, err := svc.IssueToken(ctx, uuid.New().String(), uuid.New().String(), nil, nil, false, "human")
	if err != nil {
		t.Fatalf("IssueToken: %v", err)
	}
