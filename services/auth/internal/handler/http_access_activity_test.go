...
// FUT-019 Phase 3 email-channel RPCs — stubbed; the activity handler never calls
// them, but they're part of the AuditServiceClient interface the fake satisfies.
...
func (e *activityTestEnv) issueActivityToken(t *testing.T, userID, tenantID uuid.UUID, markAdmin bool) string {
	...
	tok, err := e.tc.svc.IssueToken(context.Background(), userID.String(), tenantID.String(), nil, roles, false, "human", nil, "")
	if err != nil {
...
func TestHTTP_Activity_NoService_Returns501(t *testing.T) {
	...
	userID := uuid.New()
	tok, err := tc.svc.IssueToken(context.Background(), userID.String(), tenantID.String(), nil, []string{"reader"}, false, "human", nil, "")
	if err != nil {
		t.Fatalf("IssueToken: %v", err)
	}
