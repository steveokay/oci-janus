...
//
// REDESIGN-001 Phase 3.2 / 9.4 (ADR-0031) — the platform is single-tenant, so
// the deployment owns exactly one tenant (the bootstrap one). Refusing a second
// CreateTenant here is the structural enforcement of CLAUDE.md's single-tenant
// banner: a misconfigured FE/CLI that tries to mint a second tenant gets a
// FAILED_PRECONDITION rather than silently corrupting the deployment posture.
// The guard is unconditional — the bootstrap CLI is the only legitimate caller.
func (h *GRPCHandler) CreateTenant(ctx context.Context, req *tenantv1.CreateTenantRequest) (*tenantv1.Tenant, error) {
	...
	// Single-tenant guard (ADR-0031). Runs before the actual INSERT so the
	// caller learns the deployment already owns its one tenant before any DB
	// write. COUNT is checked TOCTOU-unsafe against the INSERT below — that's
