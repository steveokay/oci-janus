// RED-FU-013 — smoke tests for buildGRPCOptions, the Phase 3.4 helper
// that threads the SingleTenantInjector through the shared interceptor
// chain. Scanner was deferred from the RED-FU-011 sweep (PR #185)
// because its interceptor chain was still inlined into Run() rather
// than threaded through a helper. RED-FU-013 (this PR) extracted the
// helper and adds the matching smoke tests.
//
// libs/tenant/bootstrap already has bufconn-based coverage for the
// post-dial FetchTenantID call itself; this file covers the per-service
// signature added in Phase 3.4 #7 (PR #175).
//
// What we verify:
