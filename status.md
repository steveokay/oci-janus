...
> Last updated: 2026-06-21 (afternoon — **FE-API-027 closed** via merge `21a2f85` (source `7bb2e09`). Single-agent wave, clean execution: custom domain CRUD shipped with 5 routes + 4 new tenant RPCs + atomic primary swap inside one tx + swappable txtLookup for verify-now. `verification_token` deliberately stripped from FE list response (token-leak protection). PATCH `is_primary:false` rejected 400 — silently demoting would orphan the host onto the wildcard fallback. **Only 2 FE-API items remain**: 032 (gc status surface — services/gc has no gRPC server, biggest scaffold cost) and 034 (SSO providers — OAuth callbacks + SAML ACS + admin CRUD, the mega-item that warrants its own sprint).).
>
