...
- [ ] **B2 — API key creation errors from `/profile` or sidebar.** Creating an API key returns an error. **Investigation steps:** (1) reproduce in browser DevTools, capture the failing request + response, (2) check `docker logs docker-compose-registry-management-1` + `docker-compose-registry-auth-1` for the timestamp, (3) check whether the polymorphic `api_keys` table change from FE-API-048 T6 (`56f5533`) — and the new HTTP /service-accounts routes from T13 (`a5fd2e6`) — broke the user-keyed creation path. Likely related to T6 / T8 / T13 migrations.
...
#### Suggested batching
- **Batch 1 (data-integrity bugs first):** B1, B3 — both are "things look wrong / are wrong" complaints that erode trust. ~1 day.
...
| FE-API-048 | **Service accounts + /api-keys access hub** | `services/auth`, `services/management`, `frontend` | DONE ✅ | Service accounts + /api-keys access hub. Three goose migrations + 9 new HTTP routes on services/auth + shadow-user principal pattern + activity facade over services/audit + four preview surfaces in the frontend hub. Spec: docs/superpowers/specs/2026-06-21-service-accounts-and-access-hub-design.md. Plan: docs/superpowers/plans/2026-06-21-service-accounts-fe-api-048.md. |
