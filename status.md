...
| FE-API-037 | **Per-repo retention policy CRUD** | `services/management`, `services/metadata` | NOT STARTED | Per-image lifecycle policy attached to a repository. New `retention_policies` table on services/metadata (PK=repo_id, FK to repositories). Policy shape: `{ rules: [{kind: "max_age_days"\|"max_count"\|"max_size_bytes"\|"dangling_grace_days", value: N}], protected_tag_patterns: ["latest", "stable", "^v?\\d+(\\.\\d+){0,2}$", ...], enabled: bool, preview_until: timestamp }`. New gRPC RPCs on metadata: `GetRepoRetentionPolicy(repo_id)`, `UpsertRepoRetentionPolicy(repo_id, policy)`, `DeleteRepoRetentionPolicy(repo_id)`. HTTP routes on management: `GET/PUT/DELETE /api/v1/repositories/{org}/{repo}/policies/retention`. **Auth**: repo `admin` or `owner` to PUT/DELETE; repo `reader` or above to GET. Mirrors how members + webhooks are gated — never platform-admin tier. UI surface: new "Retention" tab on the repo detail page next to Tags / Members / Settings. |
| FE-API-038 | **Retention policy dry-run + preview window** | `services/management`, `services/gc`, `services/metadata` | NOT STARTED | `POST /api/v1/repositories/{org}/{repo}/policies/retention/dry-run` accepting a candidate policy in the body (NOT yet persisted). Returns `{ would_delete: [{manifest_digest, tags, pushed_at, size_bytes, reason}], total_count, total_bytes, protected_skipped: [...] }` so the operator sees the diff before enabling. **24h preview window**: when a policy is first enabled or rules change, `preview_until = NOW + 24h` is set; the executor (FE-API-040) skips deletion when `preview_until > NOW` but DOES write a "would have deleted" entry to the run log. UI shows a banner during the preview window. Mirrors the existing `quota_preview` UX guard pattern. Auth: same as FE-API-037 (repo `admin`+ for the dry-run; everyone with read access can see the preview banner). |
