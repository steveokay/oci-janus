...
> Last updated: 2026-06-19 (afternoon — **Beacon frontend rebuild Sprints 0–5 shipped** on `feat/frontend-rebuild`: dashboard, repositories list + detail, tag detail with scan/build, security IA, activity stub, members (org + repo), webhooks (list + detail + CRUD + deliveries + test + rotate), profile + API keys. Sprint 6 (admin tenants UI) in flight. FE-API agent wave landed: **FE-API-004 / 011 / 012 / 013 / 016 / 020 closed** across services/auth, services/audit, services/metadata, services/management (commits on worktree branches; merge into `feat/frontend-rebuild` pending). FE-API-005 and FE-API-006 confirmed DONE backend-side (per-repo members route + `Repository.description`). Frontend-only tracking moved out of this file to `frontend/FE-STATUS.md`. FE-API-002 / 003 / 007 / 008 / 009 / 014 / 015 / 017 / 018 / 019 still open.).
>
...
| FE-API-002 | Per-tag manifest detail endpoint | `services/management`, `services/metadata` | NOT STARTED | `GET /api/v1/repositories/{org}/{repo}/tags/{tag}/manifest` returning layer digests + sizes + media types + OS/arch. UI surface: layer drawer on the detail page; power-user "what's in this image" view. |
| FE-API-003 | Per-tag signing-status endpoint | `services/management`, `services/signer` | NOT STARTED | `GET /api/v1/repositories/{org}/{repo}/tags/{tag}/signature` returning Cosign/Notary verification result + signer identity. UI surface: signing pill per tag-table row. `services/signer` already issues `SignManifest`/`VerifyManifest` over gRPC — needs an HTTP route on `services/management`. |
| FE-API-004 | Repo-scoped recent activity query | `services/management`, `services/audit` | DONE ✅ | `GET /api/v1/repositories/{org}/{repo}/activity?since&limit&page_token&event_types` via new `GetRepoActivity` gRPC RPC on services/audit + `repo_activity.go` HTTP route. event_types allowlist: push.completed/failed, manifest.deleted, tag.deleted, scan.completed, scan.policy_blocked, image.signed (unknown values rejected before SQL). Keyset pagination via base64url(RFC3339Nano\|uuid) cursor over the `(tenant_id, occurred_at DESC)` index. `payload_summary` returns selected stable fields only. 25 new tests. **Known follow-up:** delete.* and image.signed consumer paths store raw JSON in `resource` and don't populate `metadata->raw->repository_name` — pre-existing consumer bug, those events won't surface until fixed. Worktree `worktree-agent-a2a37a9f5f700fdf5` (commit `8227ae7`). |
...
| FE-API-009 | Workspace name / metadata for the current tenant | `services/management`, `services/tenant` | NOT STARTED | The sidebar `WorkspaceSwitcher` currently maps the dev tenant UUID to "Default" via a hardcoded lookup table. Needs `GET /api/v1/workspace/me` returning `{ name, slug, host }` for the active tenant. Same Sprint 3 timeframe as multi-workspace switching. |
...
| FE-API-011 | **`GET /api/v1/users/me`** — current user metadata | `services/auth` | DONE ✅ | Returns `{user_id, username, email, display_name, created_at, last_login_at, tenant_id, roles, memberships[]}`. `last_login_at` updated on every successful login. Migration `20260619000001_add_user_profile_fields.sql` adds the three new columns. Worktree `worktree-agent-a1c05587e76af593a` (commit `cc4b710`). |
| FE-API-012 | **`PATCH /api/v1/users/me`** — update display name + email | `services/auth` | DONE ✅ | Optional `display_name` (1-128 chars, no control chars) + `email` (format-validated). Returns updated user (same shape as GET). Same worktree as FE-API-011. |
| FE-API-013 | **`POST /api/v1/users/me/password`** — change password | `services/auth` | DONE ✅ | Body `{current_password, new_password}`. Policy: 12+ chars, ≥1 upper/lower/digit/symbol; reject new == current. argon2id verify + re-hash. Redis rate limit 5/hour per user-id (counter increments before verify to block CPU brute-force; fail-open if Redis down; 429 on overflow). JTI revocation: per-user Redis SET tracks active tokens; on password change each member written to `jwt:revoked:<jti>` (TTL = token TTL). Returns 204 on success, 401 generic on wrong current_password. Same worktree as FE-API-011. |
...
| FE-API-020 | **Per-tenant security overview snapshot** | `services/management`, `services/metadata` | DONE ✅ | `GET /api/v1/security/overview` returns `{open_vulnerabilities_total, severity_counts, scan_coverage:{tags_total, tags_scanned, percent}, recent_scans_24h, days_since_last_scan}`. New `GetSecurityOverview` gRPC RPC on services/metadata. Single three-CTE query (vuln_sums + tag_counts + scan_recency) — one round-trip, tenant filter in each CTE for auditability. `days_since_last_scan = -1` sentinel for never-scanned tenants. Same worktree as FE-API-016. |
