...
|---|---|---|---|---|
| FE-API-001 | Tag size on `ListTags` response | `services/management`, `services/metadata` | NOT STARTED | Add `size_bytes` to `TagResponse` so the detail-page tag table can render per-tag size (and flag fat images at a glance). Currently the row shows tag/digest/last-push only. Pick this OR FE-API-002 — both serve the same UI need. |
...
| FE-API-004 | Repo-scoped recent activity query | `services/management`, `services/audit` | NOT STARTED | `GET /api/v1/repositories/{org}/{repo}/activity?since=…` returning push / delete / scan events scoped to one repo. UI surface: "Recent activity" panel on the detail page. `registry-audit` has the events; today only `GetBuildHistory` (a narrow slice) is exposed via management. |
| FE-API-005 | Per-repo member list | `services/management`, `services/auth` | NOT STARTED | `GET /api/v1/repositories/{org}/{repo}/members` returning `{ user_id, role }` for users with `scope_type='repo'` grants on this repo. UI surface: "Permissions" panel on the detail page. Org-scope member list already exists in management RBAC routes; this is the per-repo variant. |
| FE-API-006 | Repository description / README field | `services/management`, `services/metadata` | NOT STARTED | Add a markdown `description` column to the `repositories` table, plumb through `RepoResponse` and `createRepositoryBody`. UI surface: README section on the detail page (read-only first, editable in Sprint 2). |
...
| FE-API-010 | **Org name on `RepoResponse`** | `services/management`, `services/metadata` | NOT STARTED | Today `RepoResponse.name` is the leaf only (e.g. `alpine`); the parent org name (e.g. `dev`) lives in `organizations.name` JOINed via `repositories.org_id`. The frontend list table can't navigate to `/repositories/{org}/{repo}` without that info — it currently falls back to a hardcoded `dev` org so the click works in the dev tenant. Two fix options: (1) add an `org` string to `proto Repository`, populate via JOIN in `services/metadata`'s `ListRepositories` and `GetRepository`, surface in `RepoResponse.org`; (2) change `RepoResponse.name` to return the full `org/leaf` form (easier diff, but breaks anyone parsing the leaf). Option 1 is cleaner. |
| FE-API-011 | **`GET /api/v1/users/me`** — current user metadata | `services/auth` | NOT STARTED | The Sprint 1e Profile page currently reads `sub` / `username` / `tenant_id` / `roles` from the JWT only. To surface display name, email, last-login timestamp, account creation date, and per-tenant memberships we need an explicit endpoint. UI surface: identity card on `/profile`. |
| FE-API-012 | **`PATCH /api/v1/users/me`** — update display name + email | `services/auth` | NOT STARTED | UI surface: inline-edit on the Profile identity card (currently read-only with a friendly "soon" footer note). |
| FE-API-013 | **`POST /api/v1/users/me/password`** — change password | `services/auth` | NOT STARTED | Body `{ current, new }`. Must enforce the same password policy as the seed migration (length + complexity). UI surface: a Password section on `/profile` with a Radix-Dialog change-password flow. |
