...
>
> Last updated: 2026-06-24 (**DSGN sweep — 16 of 24 design-review items shipped today + yesterday**). Today: PR #55 (DSGN-006 repo Settings sub-sections + sticky `xl:` ToC via `IntersectionObserver`), PR #56 (DSGN-005 first-run guidance + v2 hybrid — stat row stays visible always + compact horizontal "Get started" strip below when `total_repos === 0` + amend commits to merge 5-card top row, drop StorageCard `md:col-span-2`, and re-add docker tag command / API-key link / "Replace your-org/your-image" hint / "Three commands to your first push" tagline so the strip carries every signal from the original v1 4-card stack in minified form), PR #57 (polish micro-batch — DSGN-010 scanner adapter sort + "Replace `<currentActiveName>` with this" button copy, DSGN-011 `/api-keys` preview flyout with localStorage persistence, DSGN-015 `CoverageCard` promoted from Overview tab to top-row + filler card dropped, DSGN-016 notifications-bell footer with "See all activity" + "Failures only" deep-link + `/activity` route's `event_types` URL-param hydration, DSGN-019 tag-detail empty-scan inline sibling-tab links + `EmptyState.description` widened to `React.ReactNode`, DSGN-020 webhook detail Pause/Resume button), PR #53 (DSGN-001 `isWorkspaceAdmin` helper + 3 call-site swaps + 5 new vitest cases). 16 of 24 DSGN now done. Remaining: DSGN-002 (sidebar IA), DSGN-008 (topbar breadcrumbs), DSGN-009 (audit-export tiles redesign), DSGN-018 (`<SecretInput>` primitive), DSGN-021 (custom-domain TXT row-expand), DSGN-023 (mobile sidebar), DSGN-024 (`<PageHeader>` primitive). New items surfaced from today's onboarding smoke test: **REM-017** (platform-admin "claim a new org" route — chicken-egg gap; admins can only bootstrap repos under the seeded `dev` org without SQL) and **FUT-011** (drive the new-user onboarding flow end-to-end through the FE; backend half verified today via curl). Both filed in `futures.md`.).
>
> Previous: 2026-06-23 (late night — **QA-002 closed** via PR #46 main fix + PR #47 deterministic regression tests, both on `fix/libs-rabbitmq-publisher-concurrency` / `test/libs-rabbitmq-publisher-nack-correlation`. PR #46 added `sync.Mutex` to `libs/rabbitmq/publisher.Publisher.Publish` to serialise the publish-then-read-confirmation sequence so concurrent callers can't crosswire each other's ACK/NACK on the shared `confirms` channel; the qa-agent review surfaced a deeper bug class (cancelled-Publish leaves a stranded confirmation that poisons the next caller), addressed by `drainStaleConfirm` synchronously inside the locked region on both ctx-done and timeout branches. PR #47 extracted a small unexported `amqpChannel` interface so unit tests can inject a fake — needed because empirical testing against the real broker showed Go's runtime FIFO ordering of select waiters masks the bug under happy concurrent publishes. Two new deterministic unit tests (`TestPublisher_MutexSerialisesConcurrentCallers` + `TestPublisher_DrainStaleConfirm`) prove the mutex + drain are load-bearing: each test fails deterministically when its protection is stripped (verified: "B entered PublishWithContext while A still held the mutex" and "B: got <nil>, want NACK error (drain did not consume A's stranded ACK)"). The broker-driven `TestPublisher_NackAckCorrelation` attempt was dropped — couldn't catch the bug because of Go runtime fairness; tombstone comment left in the integration test file. Production impact: every caller across the 14 services that share a `*Publisher` (audit / scan / push / signing / retention / webhook / RBAC / SA lifecycle event flows) now correctly correlates confirmation-to-publish. Follow-ups (Close() lock + configurable PublishTimeout, surfaced by qa-agent review) tracked as QA-002a/b in `futures.md`.).
>
> Previous: 2026-06-23 (late evening — **post-OSS hygiene batch 1 + deep review batch landed**. PR #43 (`docs/tracker-complete-open-issues`) listed HYG-001..008 in status-tracker.md as the post-OSS contributor-onboarding surface. PR #44 (`feat/oss-hygiene-batch-1`) shipped **HYG-002** (CODE_OF_CONDUCT.md — Contributor Covenant 2.1), **HYG-003** (.github/SECURITY.md — placed in `.github/` to avoid case-collision with the existing lowercase `security.md` audit log), **HYG-004** (.github/ISSUE_TEMPLATE/bug_report.yml + feature_request.yml + config.yml + PULL_REQUEST_TEMPLATE.md). Remaining HYG-001/005/006/007/008 (visual + GitHub Settings UI work) stay in status-tracker.md. Same day: three parallel review agents (design / quality / architecture) did a deep cross-cutting review — 74 findings (24 DSGN + 28 QA + 22 ARCH) curated into [`futures.md`](futures.md) "Review batch — 2026-06-23" with Tier 1/2/3 prioritisation. Full per-finding detail (file paths + line numbers + proposed fixes) persisted to [`.claude/reviews/`](.claude/reviews/). The biggest architectural risks surfaced: ARCH-002 (no transactional outbox for RabbitMQ — broker outage between DB commit and publish silently drops `push.completed`/`scan.completed`/audit events), ARCH-001 (documented PostgreSQL RLS layer only exists on `audit_events`, not the 5 other tenant-scoped DBs), QA-002 (`libs/rabbitmq/publisher.Publish` not goroutine-safe — silent ACK correlation under concurrent load), QA-001 (`signatures` table has no `tenant_id`), and ARCH-005 (no production-mode config validation). PR opened on `chore/track-review-findings`.).
>
