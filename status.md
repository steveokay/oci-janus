...
> Last updated: 2026-06-21 (late evening — **FE-API-034 closed** (OAuth) + SAML deferred to 501 stubs. Merge `4e3d939` (source `23760f3`) on `feat/sprint-9`. Hand-rolled OAuth flow (no x/oauth2 dep), PKCE S256 mandatory, single-use state via DELETE..RETURNING, constant-time compare, open-redirect guard on `?next=`, `email_verified=true` required for auto-provisioning. JWT returned via 302 to `{next}?sso_token=<jwt>` (FE swaps to authStore + history.replaceState; bounded by 5-min JWT TTL). Per-tenant admin CRUD on services/auth REST; `client_secret` AES-256-GCM encrypted before storage, never returned. SAML routes return 501 — `crewjam/saml` integration is its own sprint. **🎉 ALL 36 BACKEND FE-API ITEMS CLOSED** — the full backlog from FE-API-001 through FE-API-036 is shipped (with SAML the only documented deferral within an otherwise-DONE item).).
>
...
| FE-API-034 | **SSO provider configuration** | `services/auth` | DONE ✅ (OAuth) / DEFERRED (SAML) | Migration `20260621000001_auth_providers.sql`: `auth_providers` (with `auth_provider_type` enum + per-canonical-type unique constraint for Google/GitHub/Microsoft), `auth_login_sessions` (PK=state, 10-min TTL, single-use via `DELETE ... RETURNING`), `users.sso_provider_id` column. **OAuth (Google / GitHub / Microsoft / generic OIDC)** shipped end-to-end: hand-rolled `net/http` against hardcoded provider endpoints, PKCE S256 mandatory for every provider, CSRF state 32-byte `crypto/rand` base64url, constant-time compare on callback, `?next=` validated (no `://`, `//`, CRLF, ≤256 chars), `email_verified=true` required for auto-provisioning. JWT returned via `302 to {next}?sso_token=<jwt>` — frontend swaps into authStore + `history.replaceState`; trade-off (token briefly in browser history) bounded by 5-min JWT TTL. **Auto-provisioning**: case-insensitive email match; new user gets empty `password_hash`, `sso_provider_id` set, `default_role` granted at `org=*` scope (sign-in works, no org access until admin grants); race-safe via `ErrAlreadyExists` re-query. **Admin CRUD** on services/auth REST (data owner): per-tenant admin configures their own providers; `client_secret` accepted plaintext + AES-256-GCM-encrypted before storage; NEVER returned in any response. Audit events: `auth.provider_created/updated/deleted` (no client_secret in payload). **SAML DEFERRED** — routes return 501 "coming soon" so dashboard doesn't 404; full `crewjam/saml` integration (metadata + sig validation + ACS POST) warrants its own sprint. 12 new tests (sso happy/replay/unverified/open-redirect×5, admin CRUD reader-403/secret-no-leak/bad-type-400/SAML-needs-metadata-400/delete). Merge commit `4e3d939` (source `23760f3`). |
