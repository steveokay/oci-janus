...
| FE-API-016 | **Severity breakdown in `/stats`** | `services/management`, `services/metadata` | NOT STARTED | Extend `StatsResponse` with `severity_counts: map[string]int32` (CRITICAL/HIGH/MEDIUM/LOW/NEGLIGIBLE → count) aggregated across the tenant's latest scan results. Today only the total `vulnerability_count` is exposed, which loses the severity signal. UI surface: dashboard tile + `/security/overview` severity ring chart. |
...
| FE-API-020 | **Per-tenant security overview snapshot** | `services/management` | NOT STARTED | `GET /api/v1/security/overview` returning a single struct with: total open vulns, severity breakdown, scan coverage % (tags scanned ÷ tags total), recent-scan count, days-since-last-scan. UI surface: `/security/overview` landing page. The "Security" sidebar group earns its slot once at least 4 of FE-API-014..020 ship. |
