...
> Last updated: 2026-06-20 (**Beacon frontend Sprint 6 shipped** and feat/frontend-rebuild merged into main via PR #14 (`2477358`). New branch `feat/sprint-7` cut from that base. `/admin/tenants` route now live: beforeLoad guard, platform-admin amber banner, PlanBreakdown tiles, TenantsTable + CreateTenantDialog + SetQuotaDialog (GB/TB toggle with byte preview) + DeleteTenantDialog (type-name-to-confirm cascade soft-delete). Backend FE-API-004 + 016 + 020 landed via integration commit `b09ba36`; FE-API-011/012/013 (`/users/me` GET / PATCH / password change) landed via merge `22fa246` (Agent A worktree branch). Fix-up commit `9c54b82` resolved two follow-ups: ListRepositories scan was missing `repo.Description` (FE-API-006 column-add caused 500 → empty list → /repositories//<name> 404; column order corrected); UpdateTenantQuota changed from UPDATE → UPSERT so platform admins can set quotas before a tenant has first-push activity. FE-API gaps remaining: 002 (manifest detail), 003 (signing status), 007 + 009 (workspace identity — agent C never restarted), 008 (notifications), 014 / 015 / 017 / 018 / 019 (security suite).).
>
