...
- **Notification-channel secret sweep gap (from FUT-019 email #288 + webhook #291 review batches):** `rotate-kek`'s `specs()` sweeps only `audit_export_configs` — neither `notification_webhook_config.secret_enc` (#291, `NOTIFY_WEBHOOK_KEY_HEX`) nor `email_transport_config` (#288, `NOTIFY_EMAIL_KEY_HEX`) is re-sealed on rotation. Both carry `kek_version` columns already; wire both into a `webhookSpecs()`/`emailSpecs()` sweep (mirror auth's `--mfa` multi-domain pattern) as part of the KEK-rotation work. Until then treat both KEKs as fixed. Full note in [`futures.md`](futures.md) FUT-019 accepted-follow-ups block.
