...
- **FUT-017** — Scan + sign on cached images (filed 2026-06-24, scope expanded same day to lock signing=Answer A; FUT-016 dependency NOW LANDED): `services/proxy` publishes `cache.populated`; `services/scanner` consumes + scans; `services/signer` participates in optional auto-sign-on-cache. Two new tabs on FUT-016's detail page + per-upstream policy editor on the cache page header. ~1 sprint
