...
| 11 | **NEW rot (found by the PR #270 qa-agent, 2026-07-05):** `ci-core.yml` `conformance` is red again on main — but a *different* root cause than #10. Failure is `dependency failed to start: container docker-compose-registry-auth-1 is unhealthy` — the auth container builds fine then fails its startup healthcheck, so conformance never runs. Entered main with commit `13a2fee` = **PR #267 (TOTP MFA merge)**; prior run (`3e52929`, 07-04 20:10) was green. Verified via `gh run list --workflow ci-core.yml --branch main --log-failed`. **Not** an OCI spec violation and **not** attributable to #270 (auth-only, and `ci-core` is path-filtered on `proto/**` so #270 never triggers conformance). Most likely a new required env var / migration / dependency the MFA merge added to `registry-auth` that the compose conformance stack doesn't provide (candidate: `MFA_SECRET_KEY_HEX` fail-closed startup). | PR #267 merge (`13a2fee`) | **Blocks the v2.0.0 tag** — main must be green ≥2026-07-07. Fix before tag: reproduce the compose auth healthcheck failure, supply the missing config/env in `infra/` conformance compose, confirm `ci-core` green on main. Owner: ci-pipeline-agent. |
