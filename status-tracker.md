...
| trackers sync (Phase 5) | plan dashboard + status-tracker + status.md + security.md + memory refresh after #192-#199 close-out. 9 shipped rows added; SEC-040/041/042 logged as Open security items; 5.5 follow-up tail noted | #200 | 2026-06-30 |
| 5.5 follow-ups (SEC-040..043) | SEC-040 tenant filter on `GetUserBySSOSubject` (new migration `20260630120000_users_sso_subject_tenant_filter.sql` over `(tenant_id, sso_provider_id, sso_subject)` partial index) + SEC-041 race-recovery subject reconciliation (drops the `byEmail.SSOSubject != ""` precondition so empty-subject rows fail closed) + SEC-042 generic rejection message (no email enumeration) + SEC-043 OAuth + SAML handler dispatch on `ErrSSOSubjectMismatch` → 401 + generic body. Pre-PR security-agent caught SEC-043 as a BLOCKER (the service-layer fixes for SEC-042 built a clean generic body but the wire surface 500'd because handlers had no explicit case for the sentinel); fix folded inline before push. 3 new regression tests including E2E `TestSSOCallback_SEC043_SubjectMismatchReturns401WithGenericBody` that drives the full callback flow. 4 SEC findings closed in one branch | #201 | 2026-06-30 |
| 6.4 | AES-256-GCM ciphertext layout gains a `Version = 0x01` prefix in `libs/crypto/aes` so future KEK rotation has a wire-format hook. Decrypt is "try v1 then fall back to legacy" (legacy ciphertexts have random nonces; ~1/256 start with `0x01` by coincidence, so strict dispatch would break those rows). Safety: a tampered v1 row routed to the legacy path uses a wrong nonce slice and GCM auth fails. 7 consumers across `services/{auth,proxy,signer,webhook,audit}` untouched (public signatures preserved). Forward-compat note for v2 author logged | #203 | 2026-06-30 |
| 6.10 | New `libs/middleware/grpc.PeerCNAllowlist` + `PeerCNAllowlistStream` interceptors wired into both unary + stream chains. `MTLS_PEER_CN_ALLOWLIST` env var (CSV) → server denies CA-signed peers whose CN is not in the list with `codes.PermissionDenied`. Option A default: empty allowlist = no enforcement (per-server opt-in). Fold added SEC-044 production-time WARN + `GRPCPeerCNAllowlistEnabled` gauge so the disabled-in-prod misconfiguration surfaces in deploy logs + alerts; SEC-045 `GRPCPeerCNDeniedTotal{method, reason}` counter so operators can alert on cross-service call attempts. Per-service `MTLS_PEER_CN_ALLOWLIST` rollout deferred (build the primitive first, opt-in per service afterward) | #204 | 2026-06-30 |
| 6.9 | `libs/auth/mtls` gains `ReloadingServerTLSConfig` + `ReloadingClientTLSConfig` wiring `tls.Config.GetCertificate` to a per-config `certCache` keyed on `(mtime, size)` fingerprint. Existing `ServerTLSConfig` / `ClientTLSConfig` delegate to the reloading variants — universal opt-out so all 13 services get hot reload for free. No fsnotify dep; CA pool stays static (years cadence); reload failures fall back to cached cert. Fold added SEC-046 doc caveat (cached fallback is wrong channel for emergency revocation — use CA pool / CRL instead), SEC-047 `TestCertCache_BadReloadFallsBackToCached` regression, and `slog.Warn` on both stat-failure-with-cache and reload-failure-with-cache so a stuck rotation is visible in the log | #205 | 2026-06-30 |
| 6.5 | Multi-key JWT signing + JWKS rotation in `services/auth`. New `services/auth/internal/service/keyring.go` (private `keyRing` type, `LoadKeyRing(dir, signingKID)`, exported `KeyRing` alias). New env vars: `JWT_KEY_RING_PATH` + `JWT_SIGNING_KID` (mixing with legacy single-key trio rejected at startup). IssueToken stamps `tok.Header["kid"]`; ValidateToken does kid-targeted lookup first with a graceful fallback for legacy tokens; JWKS enumerates every public key. Fold added SEC-048 hard cap `maxKeyRingSize = 16` + `registry_auth_jwt_kid_fallback_total{reason}` counter (DoS amplification bound), SEC-049 pubkey SHA-256 boot log + mtime-based default signer (lex-greatest mis-selected when operators used semantic names like `prod-a/b/c`), qa-agent's `TestLoad_RejectsMixedJWTConfig` regression | #206 | 2026-06-30 |
| 6.7 | 60s Redis cache for API-key Argon2 verification. Cache key = `apikey:valid:<keyID>:<sha256-hex-secret>` (secret hash required so a stolen keyID alone cannot surface a HIT). NO negative cache. Cache HIT path still hits the DB — `s.apiKeys.GetByID` runs unconditionally and `applyKeyChecksFromCache` re-runs every row-state gate (expiry/disabled/SA-disabled/cross-tenant/scope-intersection) from the LIVE row so a stale cache cannot outlive a revocation. Cache invalidated on `DeleteAPIKey` / `SetUserDisabled` / `ServiceAccountService.SetDisabled` / `Delete`. Fail-open on Redis down (cache is optimisation, not security boundary). 10 unit tests including `HitBypassesArgon2` that poisons the stored hash after first call to prove Argon2 is genuinely skipped | #207 | 2026-06-30 |
| 6.12 | Tamper-evident hash-chain on `audit_events`. Each row carries `prev_hash` + `row_hash` (`sha256(prev_hash \|\| canonical_row_bytes)`) and a `chain_seq BIGINT GENERATED ALWAYS AS IDENTITY`. Per-tenant chain (advisory lock by `tenant_id` serialises inserters). VerifyChain walks the linked-list and returns the first tamper/fork. JSONB canonicalisation + microsecond truncation on `occurred_at` so the verifier replays identical bytes. **SEC-050 HIGH BLOCKER (pre-PR security-agent):** initial design used a writable `audit_chain_tip` table; granting UPDATE on it to `registry_audit_app` defeated tamper-evidence because a compromised audit service could rewrite the tip + INSERT a forged row chained off it without the verifier noticing. Redesigned inline: drop the tip table; derive tip from `SELECT row_hash FROM audit_events ORDER BY chain_seq DESC LIMIT 1`. `registry_audit_app` keeps INSERT-only on `audit_events`. SEC-051/052 follow-ups noted (pre-migration row warning, JSON edge cases) | #208 | 2026-06-30 |
...
- 6.12 follow-ups — SEC-051 (LOW, pre-migration rows silently unverifiable; add a `VerifyChain` info log + operational warning) + SEC-052 (INFO, `canonicaliseJSON` NaN/Inf/>2^53 edge cases — currently theoretical)
...
> **Last updated:** 2026-06-30 — Phase 6 close-out sync. 6 of 8 sub-items shipped in a parallel agent fan-out (#203-#208); SEC-044..050 all logged + RESOLVED; 6.8 + 6.11 held for design pass.
