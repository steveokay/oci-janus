...
**Trivy engine sidecar (scanner-engine-decoupling Phase 1, 2026-07-17):** Trivy no longer ships baked into the `registry-scanner` image. It runs as a co-located `trivy-engine` sidecar (`infra/scanner-plugins/engine-server`, a thin stdlib HTTP wrapper — `POST /scan` execs the Trivy CLI against a rootfs, `GET /healthz` — packaged `FROM aquasec/trivy:0.71.2`). The `trivy-adapter` shim flattens the manifest's layers onto a shared `scan_work` volume and POSTs the flattened path to the sidecar at `TRIVY_ENGINE_URL` instead of exec'ing Trivy directly; the Trivy vulnerability DB and its pre-warm step now live inside the sidecar's entrypoint, not the scanner's. A sidecar that's down surfaces as a distinct `engine_unreachable` adapter error rather than a generic scan failure. **Bumping the Trivy version is now a rebuild of the small `trivy-engine` image** (`infra/scanner-plugins/engine-server/Dockerfile --build-arg ENGINE=trivy`), not a `registry-scanner` rebuild — trust root shifted from an in-image SHA256 checksum to image-digest pinning on the sidecar. Grype stays baked into the scanner image until Phase 2. Config: `TRIVY_ENGINE_URL` (sidecar base URL, e.g. `http://trivy-engine:8090`) and `SCANNER_SCAN_WORK_DIR` (shared rootfs staging directory, backed by the `scan_work` volume in Compose / an `emptyDir` in the Helm pod spec). `GetScannerHealth` (and the BFF `GET /api/v1/admin/scanners/health`) now report `active_adapter_engine_reachable` + `active_adapter_engine_detail`, and the dashboard scanner health card shows a "degraded" badge when the engine sidecar can't be reached. See `docs/superpowers/specs/2026-07-17-scanner-engine-decoupling-design.md` for the full design.
