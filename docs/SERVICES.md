...
**Activity-feed enrichment (`source_ip` + `api_key_id`, FUT-088 follow-up, 2026-07-16):** the principal activity feed (`GET /access/activity`) populates `status` (from the audit row's `outcome`), `source_ip`, and `api_key_id` for the `service_account.*` lifecycle events registry-auth publishes. Capture is context-scoped: each SA mutation handler stamps the trusted-proxy-resolved client IP (`remoteIP(r)`) and the authenticating API-key id (`Claims.KeyID`, non-nil only on the `Bearer key.…` path) into the request `context`; the auth audit emitter reads them via `service.RequestMetaFromContext` and stamps `SourceIP`/`APIKeyID` onto `ServiceAccountLifecyclePayload`. registry-audit stores `source_ip` in the existing `audit_events.actor_ip` column (surfaced back through `NotificationRow.ActorIP` → `metadata["source_ip"]`) and keeps `api_key_id` in the event metadata JSON. `api_key_id` is intentionally blank for browser/JWT-driven admin mutations (no API key authenticated them). Enrichment of the **core** push/pull/delete events with `source_ip`/`api_key_id` is a deferred Phase 2 (needs a `ValidateAPIKey` key-id return + core event-payload plumbing).

**Service-account provenance (`origin`, 2026-07-16):** `service_accounts.origin TEXT NOT NULL DEFAULT 'manual'` records how an SA was created — `'manual'` (the normal admin path) or `'mcp-connect'` (minted by the MCP one-click connect card, named `mcp-agent-<base36>`). A migration backfills existing `mcp-agent-%` rows to `'mcp-connect'`. `origin` is accepted (validated to that closed enum, else 400) + defaulted on `POST /api/v1/service-accounts`, echoed on the SA HTTP responses (`saToResponse`), and carried on the read model — `ServiceAccountSummary.origin` (proto field 10) → the management BFF SA DTO. The FE badges MCP SAs + offers a **Settings › Agents** view (which also hosts the one-click "Connect an AI agent (MCP)" card, moved there from Integrations) filtered client-side to `origin='mcp-connect'` (last-used from the already-wired `MAX(api_keys.last_used_at)` aggregate). The `*:read` MCP scopes remain advisory (enforcement is a deferred follow-up).
...
**Trivy + Grype engine sidecars (scanner-engine-decoupling Phase 1 2026-07-17 + Phase 2 2026-07-17):** Trivy and Grype no longer ship baked into the `registry-scanner` image — the scanner image bakes **no engine binaries at all**, only the 4 adapter shims (`trivy-adapter`, `grype-adapter`, plus the other two) and clair's HTTP client. Both engines run as co-located sidecars (`infra/scanner-plugins/engine-server`, a thin stdlib HTTP wrapper — `POST /scan` execs the engine CLI against a rootfs, `GET /healthz`) built from the same `Dockerfile` via a distinct `ENGINE` build-arg: `trivy-engine` (`FROM aquasec/trivy:0.71.2`, port 8085) and `grype-engine` (`FROM anchore/grype:0.116.0`, port 8086 — the two sidecars share the pod's network namespace, so they need distinct ports). The `trivy-adapter`/`grype-adapter` shims flatten the manifest's layers onto a shared `scan_work` volume and POST the flattened path to their respective sidecar (`TRIVY_ENGINE_URL` / `GRYPE_ENGINE_URL`) instead of exec'ing the engine CLI directly; each engine's vulnerability DB and pre-warm step live inside that sidecar's own entrypoint, not the scanner's. A sidecar that's down surfaces as a distinct `engine_unreachable` adapter error rather than a generic scan failure. **Bumping either engine's version is now a rebuild of that small `<engine>-engine` image** (`infra/scanner-plugins/engine-server/Dockerfile --build-arg ENGINE=trivy|grype`), not a `registry-scanner` rebuild — trust root shifted from an in-image SHA256 checksum to image-digest pinning on the sidecar. Config: `TRIVY_ENGINE_URL` / `GRYPE_ENGINE_URL` (sidecar base URLs) and `SCANNER_SCAN_WORK_DIR` (shared rootfs staging directory, backed by the `scan_work` volume in Compose / an `emptyDir` in the Helm pod spec). `GetScannerHealth` (and the BFF `GET /api/v1/admin/scanners/health`) report `active_adapter_engine_reachable` + `active_adapter_engine_detail` for whichever adapter is active, and the dashboard scanner health card shows a "degraded" badge when the active engine's sidecar can't be reached. Remaining out-of-scope items (cosign-verify of engine images, the A3 registry-pull model, operator-installable engines at runtime) tracked as FUT-090. See `docs/superpowers/specs/2026-07-17-scanner-engine-decoupling-design.md` for the full design.
