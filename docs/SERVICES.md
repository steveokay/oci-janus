...
**Service-account provenance (`origin`, 2026-07-16):** `service_accounts.origin TEXT NOT NULL DEFAULT 'manual'` records how an SA was created — `'manual'` (the normal admin path) or `'mcp-connect'` (minted by the MCP one-click connect card, named `mcp-agent-<base36>`). A migration backfills existing `mcp-agent-%` rows to `'mcp-connect'`. `origin` is accepted (validated to that closed enum, else 400) + defaulted on `POST /api/v1/service-accounts`, echoed on the SA HTTP responses (`saToResponse`), and carried on the read model — `ServiceAccountSummary.origin` (proto field 10) → the management BFF SA DTO. The FE badges MCP SAs + offers a Settings › Connected Agents view filtered client-side to `origin='mcp-connect'` (last-used from the already-wired `MAX(api_keys.last_used_at)` aggregate). The `*:read` MCP scopes remain advisory (enforcement is a deferred follow-up).
