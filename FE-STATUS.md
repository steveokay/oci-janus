...
| FE-API-051 | MFA (TOTP step-up) — Tier-1 #1 core | DONE ✅ (2026-07-05, PR #267) | `MFACard` on `/profile` (moved from `/settings/account` by the #274 IA split) with enrol/disable/regenerate dialogs (`qrcode.react` QR + backup-code reveal/download), `MFAChallenge` OTP step on `/login` (two-step stateless-challenge flow), forced-enrolment redirect when `require_mfa` policy is on, and the admin "require MFA for all members" toggle in `PoliciesPanel`. `lib/api/mfa.ts` + `apiClientRaw` for the pre-auth challenge call. **Deferred (Tier-1 #1 residual in `futures.md`):** WebAuthn (active-session list shipped in FE-API-052). |
| FE-API-052 | Active session list + per-row revoke — Tier-1 #1 residual | DONE ✅ (2026-07-05, PR #270) | `SessionsCard` on `/profile` (moved from `/settings/account` by the #274 IA split; below `MFACard`): lists live sessions with device label, IP, last-active + a "This device" badge on the current row; per-row revoke via `ConfirmDestructiveDialog`; "sign out all others" action; current-session revoke clears `authStore` + routes to `/login`. `lib/api/sessions.ts` (`useSessions`/`useRevokeSession`/`useRevokeOtherSessions`, unwraps `{sessions:[]}`). Backend: new `user_sessions` table + stable `sid` JWT claim + fail-closed `revoke:sid` gate (see `status.md`). |
| FE-API-054 | UI polish batch — UIR-1..10 (2026-07-04 sweep tail) | DONE ✅ (2026-07-05, PR #279) | 10 deferred nits from the four-agent UI review: GC caption gated on a real timestamp (UIR-1); HealthCard pulse restricted to non-success tones (UIR-2); retention "based on latest N runs" caveat (UIR-3); per-cell pending on the notification matrix (UIR-4); login SSO buttons disabled + "coming soon" (UIR-5); access-review owner UUID shortened/tooltip/copy (UIR-6, partial — full name resolution needs a BFF join, tracked in `futures.md`); dead topbar `sticky`/`backdrop-blur` dropped (UIR-7); new `--color-highlight-fg` token for the unread badge (UIR-8); PoliciesPanel save → sonner toasts (UIR-9); labelled "Copy secret" button (UIR-10). All 4 FE gates green (294 tests). |
| FE-API-053 | Auth-flow + profile UX polish (login lockout reveal, dup-key message, session pagination) | DONE ✅ (2026-07-05, PR #275 + #277) | `/login` now shows a real "temporarily locked, try again in ~N minutes" message on `423 ACCOUNT_LOCKED` (via `lib/auth/lockout-message.ts`; generic no-leak error preserved for every other failure). Create-API-key dialog maps `409` to "a key named X already exists" instead of the generic error. `SessionsCard` on `/profile` gains compact client-side pagination (5 rows/page, Previous/Next + "Showing X–Y of Z"; current device stays on page 1). Backend counterparts in `status.md` (PENTEST-005 scoped reversal + FUT-003 idle-revoke fix). |
| FE-API-055 | Helm chart detail — Chart tab (FUT-022) | DONE ✅ (2026-07-06, branch `feat/fut-022-helm-chart-detail`) | New **Chart** tab on `/repositories/$org/$repo_/tags/$tag`, gated on the tag's `artifact_type === "helm"`. Renders `Chart.yaml` metadata (name / version / appVersion / description / type / keywords / maintainers / dependencies) plus `values.yaml` inline (read from the chart's content layer) without pulling the chart. `useChart` hook against BFF `GET .../tags/{tag}/chart`; `ChartPanel` component. BFF resolves tag→manifest and fetches the Helm config + content-layer blobs via the new generic `CoreService.GetBlob` gRPC (see `status.md`). Empty/error/non-Helm states handled (tab only shows for Helm artifacts). |
| FE-API-056 | Helm chart download — Download button on the Chart tab (FUT-022) | DONE ✅ (2026-07-06, branch `feat/helm-chart-download`) | One-click **"Download chart (.tgz)"** button on the tag-detail **Chart** tab (`/repositories/$org/$repo_/tags/$tag`, Helm artifacts only). Streams the chart's packaged content layer to the browser, byte-identical to `helm pull`. `useDownloadChart` hook against BFF `GET .../tags/{tag}/chart/download` (streams with `Content-Disposition: attachment`); button on `ChartPanel`. Backend: new server-streaming `CoreService.GetBlobStream` gRPC + streaming BFF endpoint (see `status.md`). |
| FE-API-057 | Email notification channel — transport panel + delivery log (FUT-019 Phase 3) | DONE ✅ (2026-07-07, branch `feat/fut-019-email-channel`) | Admin-only `EmailTransportPanel` on **Settings › Notifications** (above the category matrix): provider select **Resend \| SMTP**, "Use Gmail" preset, conditional credential fields (secrets render `•••• configured` when stored, empty submit keeps them), from-address/name, enable toggle, **Send test email** with inline `last_test_*` result. The **Email** matrix column is now unlocked (live `ChannelToggleCell`); the **Webhook** column stays locked. New topbar **✉️ delivery-log dropdown** (`EmailActivityMenu`, before the 🔔 bell) listing the current user's recent `email_deliveries` (category / subject / to-address / status badge / relative time) with empty + not-configured states. `lib/api/email-transport.ts` (`useEmailTransport`/`useUpdateEmailTransport`/`useSendTestEmail`) + `lib/api/email-deliveries.ts` (`useEmailDeliveries`). Backend: new audit email transport + send loop + auth `ResolveUserEmails` RPC (see `status.md`). |
| FE-API-058 | Webhook notification channel — transport panel + unlocked matrix column (FUT-019) | DONE ✅ (2026-07-08, branch `feat/fut-019-webhook-channel`) | Admin-only `NotificationWebhookPanel` on **Settings › Notifications** (beside the `EmailTransportPanel`): webhook **URL** field, HMAC **secret** (renders `•••• configured` when stored, empty submit keeps it), enable toggle, and **Send test webhook** with inline `last_test_*` result. The **Webhook** matrix column is now unlocked (live `ChannelToggleCell`) — **admin-editable, read-only for non-admins** — overlaying the tenant-level `enabled_categories` (the last locked channel; the per-user webhook flag is retired). `lib/api/webhook-config.ts` (`useWebhookConfig`/`useUpdateWebhookConfig`/`useSendTestWebhook`). Backend: new audit `notification_webhook_config` + `notification_webhook_deliveries` + send loop (HMAC-SHA256 + SSRF-blocked, patterns from `services/webhook`); no new mTLS peer edge (see `status.md`). |
| FE-API-059 | Ephemeral PR registries — admin config panel + active-namespaces list (FUT-023 PR B) | DONE ✅ (2026-07-10, branch `feat/fut-023-pr-registry-frontend`, PR #297) | New **Settings › Integrations** tab (`_authenticated.settings.integrations.tsx`), global-admin-gated in the settings layout. `PRRegistryPanel` mirrors `NotificationWebhookPanel`: enable toggle, copyable webhook receiver URL (`CopyButton`; empty-state hint when `PUBLIC_BASE_URL` is unset), **write-only** HMAC signing secret (`•••• configured` placeholder when stored, empty submit keeps it), promote-target-org `Select` sourced from the caller's visible orgs (`useRepositories` distinct orgs; `None` sentinel), and a targeted **`409` → "set `PR_REGISTRY_KEY_HEX` first"** toast. `PRNamespacesList` — read-only active-namespace table (provider / source repo / PR # / org / created) with empty + error states. Both render `null` for non-admins (defense in depth). `lib/api/pr-registry.ts` (`usePRRegistryConfig`/`useUpdatePRRegistryConfig`/`usePRNamespaces`). Manual FE teardown out of scope (webhook-driven). 8 vitest cases. Backend: FUT-023 PR A + SEC-085 guard (see `status.md`). |
...
| `/settings/workspace` | scan policy + `/admin/gc/*` + retention runs + scanner adapters + `/deployment-info` | REDESIGN-001 Phase 4.2.c/d — workspace config hub. **Single mode: hosts the GC surface** (`#gc` → `GCCard`: schedule + run history + run-now; `#retention` → `RetentionCard`) plus scanner adapters + deployment info, since single mode has no Platform tab |
| `/settings/platform` | `/admin/tenants` + `/admin/gc/*` + retention runs + scanner adapters + `/deployment-info` | REDESIGN-001 Phase 4.2.d — multi-mode only, global-admin gated (tab hidden + route guard). **Multi mode: hosts the GC surface** (`#gc`/`#retention`, same cards). Legacy `/admin/tenants` + `/admin/scanner` redirect here with matching hashes |
...
- ~~**FE-API-034** — SSO admin config UI~~ ✅ DONE — see `FE-API-064` below (global-admin editable SSO panel, OAuth kinds; SAML editing still deferred).
...
| Batch | Scope | Status |
|---|---|---|
| **1 — Bug batch** | 12 actively-wrong/misleading items: SA-keys expiry column mislabeled "Last used"; access-review Revoke had no confirmation; repo search false "no matches" over unfetched infinite-query pages; tags filter zero-match rendered a blank table; notifications panel hung on "Loading…" on error; StatCard infinite skeleton on absent values; login discarded the `?from` deep-link; DeploymentInfoCard hardcoded "HTTPS termination" badge; GC runs table froze after Run-now (no poll); Settings eyebrow hardcoded "Account"; dead `docs.example.com` footer link; theme "System" matchMedia listener desync | ✅ DONE — PR #257 (merged 2026-07-04) |
| **2 — A11y + consistency** | Sidebar focus rings + `aria-current` + labeled `<nav>` landmarks; tag-row nested-link semantics (mirror repositories-table); notifications bell menu→popover semantics; migrate 4 hand-rolled destructive dialogs to `ConfirmDestructiveDialog`; timestamp unification (raw `toLocaleString` in audit-export/TrustPanel, missing absolute-time `title` tooltips); ReviewPanel raw Tailwind palette → `--color-*` tokens; webhook `failed` tone split from `pending`; used/quota string + H1-scale unification; extract shared `SSOReadOnlyCard` | ✅ DONE — PR #258 (merged 2026-07-04) |
| **3 — Capability** | Sidebar Dashboard item (icon already imported); URL-driven `?tab=` on repo/tag detail; in-page section nav for long settings tabs (anchors exist, unexposed); permission-aware disabling of platform-admin CTAs (no more type-to-confirm→403); API-key expiry-urgency badges; activity feed pagination + type-filter URL persistence; dashboard manual-refresh affordance; repo-table sorting | ✅ DONE — PR #259 (merged 2026-07-04) |
| Deferred / smaller | GC "Unknown"+disclaimer caption; HealthCard pulse on healthy; retention-card 100-row-slice footnote; notification-toggle optimistic updates; SSO stub buttons visibly disabled; ReviewPanel owner UUID → `UserCell`; topbar sticky/blur no-op; hardcoded `text-white` badge; PoliciesPanel inline banners → toasts | ✅ filed as `futures.md` → UI-REVIEW-2026-07 (UIR-1..10); the `startsWith` path-boundary fix shipped in batch 2 |
...
- `FE-API-061` Environments overview — `/repositories` is now a card-per-org **environments overview** (orgs read as dev/stage/prod) drilling into a new `/repositories/$org` per-environment repo list. New `GET /api/v1/orgs` (→ `metadata.ListOrgSummaries`: per-org `repo_count` + `storage_used_bytes` + nullable `last_activity_at`) feeds `useOrgs`/`OrgCard`; the per-env list reuses `RepositoriesTable` scoped via a new BFF-side `?org=<name>` filter on `GET /api/v1/repositories`. Single-org deployments redirect the overview straight to `/repositories/$org`. Sidebar label + `org` vocabulary unchanged (not "environment" — wrong in multi mode). Cross-environment promotion/drift matrix deferred to futures.md **FUT-077**. Design/plan under `docs/superpowers/{specs,plans}/2026-07-11-environments-overview*`.
- `FE-API-062` Unified artifact catalog — per-repo **Type** badge + **Images**/**Charts** filter chips on the per-environment repo list (`/repositories/$org`), driven by the new `artifact_types: string[]` on `GET /api/v1/repositories` (shared `ArtifactTypeBadge`). Org cards gain an image/chart split from `image_repo_count` + `helm_repo_count` on `GET /api/v1/orgs`. `/helm` retired — redirects to `/repositories` and the sidebar item is removed (images + charts now live in one filtered tree). Backed by proto `Repository.artifact_types` + `ListOrgSummaries` per-type counts. Design/plan under `docs/superpowers/{specs,plans}/2026-07-11-unified-artifact-catalog*`.
- `FE-API-063` **SSO login wiring (FUT-084)** — the login page now activates SSO. `SSOButtons` fetches the configured providers from `GET /api/v1/auth/providers` (new `useAuthProviders` hook in `lib/api/sso.ts`) and renders one live button per enabled provider (no more hardcoded-disabled "coming soon" set); clicking starts the full-page OAuth/SAML dance via `beginSSOLogin` → `/auth/{oauth,saml}/{id}/start?next=/login`. login's `beforeLoad` consumes the `?sso_token=<jwt>` the auth service hands back into the in-memory `authStore` and bounces to the return path stashed in `sessionStorage` (`prepareSSOLogin`/`consumeSSOReturnTo`, open-redirect-guarded); `replace:true` keeps the token JWT out of browser history; a malformed token is dropped and falls through to the password form. When no providers are configured the whole block (incl. the "or sign in with credentials" divider, moved into `SSOButtons`) renders nothing, so a password-only deployment is unchanged. **Login-side only** — the SSO admin-config surface is now `FE-API-064`. TDD: 11 tests (helper URL/sessionStorage round-trip + guard, component render/click/empty, full-router beforeLoad handoff happy + malformed). Shipped FUT-084. NOTE: a follow-up fix (`/api/v1/auth` proxy entry, PR #335) was needed for the provider fetch to actually reach registry-auth — without it `/api/v1/auth/providers` fell through to the BFF and 404'd, so the buttons never rendered.
- `FE-API-065` **Repo rename + transfer (repo Settings › General complete)** — two new editors on the repo Settings General section, completing the deferred half of Tier 2 #2. **Rename** (`RepoRenameSection`): inline `org/<name>` input with client-side allowlist validation, a confirm dialog (rename changes pull URLs), and navigation to the repo's new URL on success; wired to `useRenameRepository` → `POST /api/v1/repositories/{org}/{repo}/rename`. **Transfer** (`RepoTransferSection`): destination-org `<Select>` populated from `useOrgs` with the current org excluded, confirm dialog, navigation to the new org on success; wired to `useTransferRepository` → `POST /.../transfer`. Both responses carry `roles_rewritten` (repo-scoped RBAC grants migrated server-side) + an optional `rbac_warning` shown as a warning toast on partial success (durable rename/transfer committed but the auth scope-rewrite failed — re-grant manually). Removed the obsolete `RepoGeneralPlaceholder` — every General editor (visibility/description/quota/rename/transfer) is now live. Backend: auth `RewriteRepoRoleScopes` (#363) + metadata `RenameRepository`/`TransferRepository` (#364) + BFF orchestration (#365/#367). TDD: 4 rename + 3 transfer FE component tests (render / validation / mutation+navigate); all 4 FE gates green (402 vitest). Shipped PRs #363/#364/#365/#367.
- `FE-API-064` **SSO admin config panel (FE-API-034 delivered)** — a **global-admin-only, deployment-wide** editable SSO surface replacing the read-only `SSOReadOnlyCard` on Settings › Workspace (single) + Platform (multi). Backend (registry-auth HTTP, no proto/gRPC/BFF): `GET/PUT/DELETE /api/v1/auth/admin/providers` gated by a new `requireGlobalAdmin` (denies SA principals + checks `users.is_global_admin`); reuses `global_sso_config` + `SSO_CREDENTIAL_KEY_HEX` (no migration). Client secret is **write-only** (`has_secret` on read; empty `client_secret` on PUT keeps the stored value; create-without-secret → 400). **OAuth kinds only** (Google/GitHub/Microsoft/Generic OIDC); SAML editing rejected (400) + not offered in the UI — stays env/SQL. Generic OIDC requires an https issuer. FE: `lib/api/sso-config.ts` (`useSSOProviders`/`useUpsertSSOProvider`/`useDeleteSSOProvider`) + `components/settings/sso-config-panel.tsx` (`useIsGlobalAdmin`-gated, mirrors `notification-webhook-panel`, `ConfirmDestructiveDialog` delete, native `<select>` for kind). RM-003 removed the *per-tenant* admin surface (§A1 gate flaw); this is the intended deployment-wide replacement. TDD: 12 backend tests (8 service + 4 httptest incl. the gate) + 5 FE. All gates green (auth build/vet/test/golangci-lint; FE lint/typecheck/375 tests/build).
