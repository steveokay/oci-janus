...
| Futures Tier 1 #3 | Signed-image admission (`require_signature`) | DONE ✅ (Phase 1) | `RepoSignaturePolicySection` toggle on the repo Settings tab next to the immutability card. Backend `GetManifest`/`HeadManifest` consult `signer.ListSignatures` when the flag is on; zero sigs → 403 DENIED. Fail-OPEN on metadata/signer blips so transient outages don't break every pull. Phase 1 contract: ANY signature passes (per-repo trusted-key allowlist deferred to Phase 2). Docs: `docs/SIGNING.md` §8. |
