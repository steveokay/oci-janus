...
| Futures Tier 1 #3 | Signed-image admission (`require_signature` + trusted-key allowlist) | DONE ✅ (Phase 1 + Phase 2) | `RepoSignaturePolicySection` toggle + `RepoTrustedKeysSection` allowlist editor on the Settings tab. Phase 2 narrows the gate to approved `key_id`s when the allowlist is non-empty; empty list falls back to "any signature passes" (Phase 1). Approve dialog with key_id validation; per-key Revoke with confirmation. Phase-1-fallback warning pill when policy is on but allowlist is empty. Docs: `docs/SIGNING.md` §8. |
| Futures Tier 1 #4 | Audit log streaming to SIEM | DONE ✅ (Phase 1) | New `/workspace/audit-export` settings page (sidebar Integrations group): format selector (syslog RFC 5424 / CEF / webhook), target URL input with format-specific validation, HMAC secret + bearer token write-only inputs with "(saved)" placeholders + revoke checkboxes, JSON filter editor, Send-test-event button with rendered-payload preview, observability banner with last-success timestamp + DLX-depth pill + last-error inline. Phase 2 (async DLX queue + replay) deferred. Docs: `docs/SIEM-EXPORT.md`. |
