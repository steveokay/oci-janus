...
| **3 — Capability** | Sidebar Dashboard item (icon already imported); URL-driven `?tab=` on repo/tag detail; in-page section nav for long settings tabs (anchors exist, unexposed); permission-aware disabling of platform-admin CTAs (no more type-to-confirm→403); API-key expiry-urgency badges; activity feed pagination + type-filter URL persistence; dashboard manual-refresh affordance; repo-table sorting | 🔄 IN REVIEW — PR #259 (`feat/ui-capability-batch-3`) |
