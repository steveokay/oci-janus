...
- **FE-API-034** — per-tenant SSO **admin config** UI (`/workspace/sso`) to add/edit providers from the dashboard. Still deferred (read-only `SSOReadOnlyCard`; providers configured via deploy-time env/SQL). NOTE: SSO **login** wiring is DONE — see `FE-API-063` (FUT-084).
