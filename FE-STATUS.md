...
| Futures Tier 1 #2 | Tag immutability (repo flag + per-tag pin) | IN PROGRESS 🚧 | Backend complete: migration + repository + RPCs + PutManifest reject. BFF: PATCH repo accepts `immutable_tags`; TagResponse carries `immutable`. FE toggle + pin affordance follow on the same branch. |
