...
| REM-011 P2 FE | Platform-admin scanner adapter page | **READY** (backend `bd4ba1d` 2026-06-21) | New `/admin/scanner` route under platform-admin guard. Cards per installed adapter (name/version/checksum/active badge), "Make active" with confirm dialog, "Run test scan" with inline result panel + duration + finding counts. Powered by FE-API-044 (list + active) / FE-API-045 (set active) / FE-API-046 (test scan) / FE-API-047 (liveness — also drives the stuck-pending degradation upgrade from heuristic to real signal). All four FE-API endpoints live + smoke-tested. |
...
> Sprint 9 sub-passes 9.1/9.2/9.3/9.4 all landed — verify-on-demand + sign-from-UI + SBOM download (`8a7271f`), workspace metadata + notifications + custom domains (`52178b1`), workspace-wide vulnerabilities + scan history (`5968bf0`), analytics + storage breakdown + admin tenant drawer + bulk tag delete (`2e983fc`). REM-011 Phase 1 backend + FE stuck-pending degradation shipped (`8debd29`). **REM-011 Phase 2 backend now live (`bd4ba1d`)** — FE-API-044..047 verified end-to-end: dev-stub ↔ trivy-adapter swap, real Trivy test scans in ~8s, selection persists across container recreate. Next FE work: REM-011 Phase 2 FE (`/admin/scanner` route) or S9.5 mop-up (FE-API-017/018/019/020/032/035).
